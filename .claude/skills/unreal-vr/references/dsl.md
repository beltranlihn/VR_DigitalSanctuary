...
- 🔴 **`\n` en string literals NO se interpreta — queda LITERAL** (bytes `\`+`n`, no salto de línea). Confirmado 2026-07-23 en un SaveGame: filas `...caldist\nCROW...` con backslash-n literal; y textos de UMG muestran `\n` en pantalla. Para saltos de línea reales: editá el string a mano en el editor (Shift+Enter) o construí el newline con un nodo. Para separar filas de un CSV, el `\n` literal sirve igual (parseás por esa secuencia). No confíes en `\n`/`\t` del DSL.
