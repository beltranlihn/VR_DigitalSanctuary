...
> 📦 **Para TRANSPLANTAR una mecánica entera a un nivel/proyecto nuevo: [`docs/MECANICAS-PORTABLES.md`](../../../../docs/MECANICAS-PORTABLES.md)** (2026-09-04) — ficha de empaque por mecánica (paquete, enchufe, API, receta, deuda), matriz de dependencias medida por MCP y el plan de extracción. Este archivo mapea assets sueltos; aquel mapea mecánicas completas.
...
| 🆕 `Core/Attracting/NS_OrbAttract_SC` | **Las particulas que van del orbe agarrado a la mano** (2026-09-24). **Duplicado de `NS_BreathParticles`** — emisor CPU de sprites con `User.SpawnRate`; se le quito la gravedad y se le agrego **`PointAttractionForce` V2** (la V2 porque su `Attractor Position` es `NiagaraPosition` y la estacion esta a 362.000 cm del origen). Params: `SpawnRate` · `Target` (mano) · `OrbColor`. **Es LA receta para 'particulas que convergen a un punto movil'.** Detalle en `blueprints/BP_Sequencer_SC.md`. | ⬜ construido y compilando, **sin ver correr** |
...
- 🫁🆕 **`Mechanics/Pacer/BP_Pacer_SC` + `M_Pacer_SC` + `MI_Pacer_SC` — EL PACER DE RESPIRACIÓN** (2026-09-25, Nico). Ante "necesito un marcapasos/guía visual de respiración (inhalar-aguantar-exhalar)": **YA EXISTE**. Reloj guía (no sensor) con 3 modos por `Mode` (Aros/Órbita/Arco), anillo perimetral con centro libre, presets, audio data-driven y API `PacerPlay/Pause/Stop/SetMode/SetTimes` + `OnPacerPhase/Cycle/Finished`. Reloj anti-deriva (`T-=Total`) con suavizado quíntico. Material = **Custom HLSL** portado del GLSL de `docs/efectos-nico/pacer/`. Puerta `bUseExternalLung` para atarlo a la respiración real (`MPC_Breath`). Tracker: [[BP_Pacer_SC]]. 🟡 compila y corre en PIE sin errores; falta visor.
...
- ✨🆕 **`Core/Light/BP_VoidField_SC` + `M_VoidDots_SC` — PROFUNDIDAD SIN GEOMETRIA** (2026-09-04, giro 2026-09-07): tres cascarones ESFERICOS concentricos con puntos procedurales (sin textura ni particulas), resueltos sobre `normalize(LocalPosition)` en vez de la UV. El paralaje entre capas es lo que hace sentir un vacio enorme, y lo produce el **giro por esfera**: `SpinSpeed0/1/2` (grados/seg, **el signo es el sentido**) y `SpinAxis0/1/2`, en la categoria `E - Giro`. Puntos con jitter por celda (rompe la grilla), `Density` para dejar celdas vacias (sin eso parece ruido, no estrellas), tamaño y brillo variados y centelleo. 3 draw calls en vez de un sistema Niagara. 🟡 el giro se verifico en el viewport; sin visor — y el paralaje SOLO se juzga en visor.
- 🌑🆕 **`Core/Light/BP_Ganzfeld_SC` + `M_Ganzfeld_SC` + `SM_GanzShell` — EL CASCARON SIN BORDE** (2026-09-04): la sala donde no encontras la esquina. Gradiente emisivo continuo sobre una esfera invertida de 9k tris, **opaco** (barato: escribe profundidad y mata el overdraw de atras), con **dither R2 medido** que elimina el banding de 8 bits, banda de horizonte opcional y respiracion por WPO. Tamaño en cm (`ShellRadius`/`ShellHeight`). Es el efecto 1.1 de la galeria y la sala donde van a vivir las demas estaciones. 🔴 **Desde 2026-09-07 solo se usa el MODO 2 (fluido)**, y ese modo se reescribio: **simplex 4D de Ashima/Gustavson en un nodo `Custom`** (`(x,y,z)` = la superficie, `w` = el tiempo — el equivalente del Translate Z de un Noise TOP), mezcla convexa de tres pesos, y el **Veil derivado del CONTORNO del propio degradado** (no es un ruido aparte, y se anima gratis). Perillas: `FlowScale` tamaño de mancha · `FlowSpeed` hervor · `FlowMix` separacion de colores · `VeilAmount` 0-1 cuanto se nota el borde · `VeilScale` ancho del borde. Las perillas de los modos 0 y 1 estan ocultas (Instance Editable off) pero el codigo de los tres sigue entero. 🟡 sin visor.
- 🔮🆕 **`Core/Light/BP_Orb_SC` + `M_Orb_SC` + `T_Matcap_01/02/03` — EL ORBE HECHO DE ALGO** (2026-09-04, Nico). Ante "necesito una esfera/orbe que se vea solida, de vidrio, de luz o iridiscente" (respiracion, sonido, la ameba, burbujas de Attracting): **YA EXISTE**. Un material unlit translucido con selector `Look` 0/1/2: **volumen falso** (bola de luz densa al centro), **iridiscente** (tono que corre con el angulo) y **matcap** (vidrio/perla/seda sin ninguna luz). Late solo por reloj. Malla y tamaño (cm) intercambiables. 🔴 El look es un escalar **por-pixel, NO static switch** (el MID dinamico no puede cambiar switches — si se necesita congelar por fill, MIC por look). Los 3 matcaps se generan con `docs/efectos-nico/gen_matcaps.py` (PNG puro). Tracker: [[BP_Orb_SC]]. 🟡 3 looks juzgados en editor, falta visor.
- 👻🆕 **`Core/Light/BP_RimShape_SC` + `M_RimOnly_SC` — FORMA QUE SOLO ES SU RESPLANDOR** (2026-09-04, Nico). Ante "necesito una forma sin masa, que aparezca al acercarse, un cascaron que se abre, algo que se disuelva desde el borde": **YA EXISTE**. Rim de silueta (`abs(N·V)` adentro), **revelado por cercania de camara** (`SphereMask`), **disolucion con filo encendido** (`DissolveThreshold`/`DissolveEdge`/`DissolveEdgeColor`) y `DepthFade` de contacto. Two-sided (se ve desde adentro). Malla intercambiable. Pensado para el cascaron de Surrounding y la sala final que se abre (animar `DissolveThreshold`/`RevealAmount` desde el director). Tracker: [[BP_RimShape_SC]]. 🟡 juzgado en editor, falta visor.
- 🎛️🆕 **`Core/Light/BP_LightPanel_SC` + `M_LightPanel_SC` — LA SUPERFICIE QUE HACE COSAS** (2026-09-04, Nico). Ante "necesito un panel/muro de luz que barra, un halo/roseton, un mandala que gira, o una pared que se enciende al acercarse": **YA EXISTE**. Un panel plano con selector `Mode` 0/1/2/3 (barrido / anillos / mandala / cercania por `SphereMask`) + mascara de borde. 🔴 Selector escalar **por-pixel, NO static switch** (el MID dinamico no puede; si se quiere la optimizacion de permutacion, 4 MICs — pendiente con Beltran). Tracker: [[BP_LightPanel_SC]]. 🟡 4 modos juzgados en editor, falta visor.
