...
281. 🔴🔴🔴 **Un síntoma que aparece SOLO CUANDO ALGO SE MUEVE acusa al sistema de coordenadas, no a la geometría.** Beltrán vio puntos cortados y culpó a los vértices de la esfera (*"en el cubo no pasaba"*); yo perdí dos pasadas mirando la malla. Lo que lo resolvió fue su segunda observación: *"sucede claramente cuando pasa de un lugar a otro moviéndose"*. Un vértice está quieto: si el corte viaja, el culpable es la **celda** del patrón procedural.
    ✅ **El control que lo probó en una sola corrida:** poner el jitter en 0 y los puntos GRANDES. No se cortó ninguno → la teselación es inocente. Con jitter, se cortaban → es el borde de celda. Un control positivo bien elegido reemplaza tres hipótesis.
    🚩 Y la causa raíz era mía: al pasar el jitter de 2D a 3D conecté el `AppendVector` de hashes **directo al multiplicador, salteando el `− 0,5`**. El desplazamiento quedó siempre positivo y del doble del rango, así que el clamp que calculaba el margen quedaba corto. **Al reemplazar un nodo por una versión de más componentes, hay que reconectar la CADENA entera, no solo el consumidor final** — el `read` de los inputs lo muestra en una llamada.

282. 💡 **Para repartir algo al azar sin que se vean patrones, el hash tiene que ser INDEPENDIENTE de los que ya usa el efecto.** Los tres colores de `M_VoidDots_SC` usan una constante de dot-product propia: reusar el hash del tamaño habría atado "los puntos grandes son azules", y el ojo caza esa correlación enseguida.

283. 🔴🔴🔴 **`DepthFade` / `SceneDepth` SOLO ven geometría OPACA. Un efecto que "reacciona a lo que hay detrás" es ciego a todos los demás translúcidos.** Un material `BLEND_Translucent` no escribe al buffer de profundidad, así que para el que lo lee **no existe**.
    Caso real (2026-09-04): `BP_FogSlab_SC` sobre el océano de nubes. Donde estaba el océano, la losa leía "no hay nada atrás hasta el infinito" → espesor máximo → **opaca**. Se veía como si tapara al océano por orden de dibujo, y no: estaba reaccionando a un vacío. Por eso `TranslucencySortPriority` no cambió nada — **el problema no era el orden, era la información**.
    ✅ **Si un efecto tiene que responder a un translúcido, la cuenta va DENTRO del material de ese translúcido.** No hay truco de sorting que lo arregle. Las dos formas: darle al translúcido sus propias perillas (rápido, independiente), o que el emisor publique sus parámetros a una **MPC** que el receptor lee — el patrón que ya usa el haz para bañar meshes (`MPC_LightShaft` + `M_BeamReceiver_SC`).
    🚩 Corolario para este toolkit: **los velos, la nube, los haces y la losa son invisibles entre sí.** Cualquier interacción entre dos de ellos hay que construirla explícitamente.
...
286. 🔴 **Aditivo sobre fondo claro SIEMPRE lava el color hacia el blanco.** Es la naturaleza del blend: suma. Mientras el fondo de la obra fue negro, el aditivo de `M_VoidDots_SC` era ideal (suma sin ordenar nada); apenas se encendió un fondo beige, los puntos azules, rojos y naranjas se vieron blancuzcos.
    ✅ **Si el color elegido tiene que verse tal cual, el material va TRANSLÚCIDO y la cadena se parte en dos:** emisivo = `color × Brightness` (el color puro), opacidad = la máscara (forma × centelleo × atenuación × vivo). Donde la máscara vale 1 se ve exactamente el color.
    ⚠ Y hay que avisar lo que se pierde: en translúcido `Brightness > 1` **ya no ilumina de más** (sin MobileHDR no hay rango), y todo parámetro que antes *oscurecía* multiplicando el emisivo ahora **desvanece** si vive en la opacidad. Son decisiones distintas, no equivalentes.

287. 🔴🔴🔴 **El nodo `Noise` con función de TEXTURA (`GradientTex`) tiene celdas de interpolación: si con eso desplazás vértices, las celdas se vuelven RELIEVE — terrazas y placas planas.** Es invisible mientras el ruido solo colorea; se hace geometría en cuanto entra al WPO.
    Síntoma reportado por Beltrán, y le costó semanas de convivir con él: *"a veces se ven geometrías o cosas cuadradas en la nube, cuadrados grandes que siguen la ondulación"*, y la pista final: ***"es claramente cuando aumento CloudHeight mezclado con NoiseScale, como que se rompe la geometría"*** — amplitud × frecuencia es exactamente la receta para que la cuantización del ruido se vea.
    ✅ **La solución: separar los dos ruidos.** El COLOR se queda con el ruido de textura (barato, y ahí sus celdas no se notan); la ALTURA usa un `Noise` propio con **`NOISEFUNCTION_GradientALU`** (analítico, sin celdas) y **su propia escala** (`HeightNoiseScale`), independiente del `NoiseScale` del color. Así se puede subir el detalle del color todo lo que se quiera sin que la geometría se rompa nunca.
...
## Cosecha 2026-09-07 — el campo de fur (estación de galería, descartada) y los 3 cuelgues del editor

300. 🔴🔴🔴 **Un `set_properties` con N propiedades sobre un actor con Construction Script pesado dispara N Construction Scripts completos.** Medido: 21 perillas × 34.000 instancias = **~18 minutos con el editor congelado** (`Responding=False`, memoria PLANA — está calculando, no fugando). Pasó dos veces en la misma jornada.
    ✅ **La receta: bajar el costo ANTES de escribir.** Poner `Count` (o el multiplicador de coste que tenga el BP) en un valor mínimo → escribir todas las demás perillas (cada CS ahora es trivial) → **poner `Count` al final**, una sola corrida cara. Y después `load_level` para que corra limpio.
    💡 Y la firma para distinguir los dos modos de cuelgue sin adivinar: **memoria PLANA + CPU subiendo = está trabajando** (esperá o cancelá); **memoria SUBIENDO sin parar = no converge** (reiniciá). El primer cuelgue de esa jornada subió 6,5 → 7,9 GB en 4 min y nunca iba a terminar.

301. 🔴🔴 **`AddInstance` (singular) en un bucle de Blueprint es inviable a partir de unos pocos miles: cada llamada ensucia el estado de render del ISM.** 40.000 llamadas colgaron el editor sin retorno.
    ✅ **Patrón correcto:** una variable **array de `Transform`**, el bucle hace `Utilities|Array|Add`, y **al final una sola llamada a `Components|InstancedStaticMesh|AddInstances`** (plural; pines `InstanceTransforms`, `bShouldReturnIndices`, `bWorldSpace`, `bUpdateNavigation`). Con eso, recargar el nivel con 34.000 instancias pasó a **3,3 s**.
    ⚠ Como el `for` del DSL tiene que ser el último statement, el "flush" se hace **dentro de la función del bucle** con `(if (>= I (- Count 1)) (AddInstances ...))`, y el `Clear` del array va en una función que corre **antes** del bucle. Así el Construction Script no se toca.

302. 🔴🔴 **Unreal INVIERTE la V al importar (OBJ/FBX): un `vt u t` en el archivo llega al material como `UV.y = 1 − t`.** Síntoma exacto y caro: el WPO pesado por `UV.y` movía y deformaba **la base** de cada hebra y dejaba la punta clavada — exactamente al revés. Nada en el grafo se ve mal; compila y corre.
    ✅ **Poner la inversión en UN solo lugar** (el shader: `h = saturate(1.0 - UV.y)`) y dejar el OBJ con la V natural. Invertir en los dos lados se cancela y volvés al bug.
    🚩 Regla general: **cualquier magnitud "a lo largo de la malla" que venga de UV necesita una verificación de orientación antes de construir encima**; el síntoma es geométrico y obvio en cuanto se mira, pero invisible en el grafo.

303. 🔴 **El diálogo de advertencias del import de malla es MODAL y bloquea el MCP entero.** Firma: `Responding=True` + memoria plana + el endpoint HTTP sin responder (es la misma firma del §"cuelgue por modal" ya documentado). Lo disparan warnings cosméticos — *"No smoothing group information"*, *"nearly zero tangents / bi-normals"*, estos últimos por filas de ancho ~0 en la punta y la base de una malla generada.
    ✅ Darle a esas filas un ancho mínimo no degenerado (≥ 0,05 cm), y **no reimportar de rutina**: cada import cuesta un cuelgue hasta que un humano cierra el cartel.

304. 🔴 **`remove_function_graph` + `add_function_graph` con el MISMO nombre en la misma llamada MCP devuelve `<Nombre>_0`** — el nombre queda reservado hasta que la llamada termina. Y peor: el grafo recién creado **no es direccionable** en esa misma llamada (*"is not valid EdGraph for property 'graph'"*), así que ni siquiera se le pueden escribir los parámetros.
    ✅ **Una llamada MCP por paso:** `remove` → (llamada nueva) `add` → (llamada nueva) `add_function_param` + `write_graph_dsl` + `compile`. Verificar el nombre con `list_graphs` antes de escribir.
    ⚠ Y **un grafo de función NO puede llamarse igual que un componente o variable del BP**: pedir `Field` en un BP que ya tiene un componente `Field` devuelve `Field_0` en silencio.

305. 💡 **Generar mallas paramétricas como OBJ de texto e importarlas con `StaticMeshTools.import_file` funciona y evita Blender entero** para formas de revolución/tiras (hebras, hojas, cintas). Se escribe `v` / `vt` / `vn` / `f` con un script, se importa, y `get_triangle_count` + `get_bounds` lo verifican en una llamada. El slot de material se llama **`defaultMat`** (no `""`). Iterar la forma cuesta segundos — pero ver §303 antes de reimportar en bucle.

## Cosecha 2026-09-07 (2ª tanda) — el túnel de anillos (BP_RingTunnel_SC)

306. 🔴🔴 **Los getters/setters de variables en el DSL se namespacean por su CATEGORÍA, no por `Default`.** Tras `set_variable_category(v, "A - Forma")`, el type_id real es `Variables|A-Forma|GetPortalRadius` (los espacios se comen); `Variables|Default|GetPortalRadius` falla con *"does not exist"* aunque la variable exista y compile. Solo las vars sin categoría (y los componentes) siguen bajo `Default`. Descubrir el nombre exacto con `find_node_types(graph, "Get<Nombre>", [])`.

307. 🔴🔴 **Un getter puro dentro de un loop se re-evalúa en CADA nodo exec que lo consume — el `(bind)` del DSL NO congela el valor.** Patrón contador `Idx` en un for-each: con `SetIdx(GetIdx+1)` ANTES del consumo, el consumidor lee el valor YA incrementado (fases 1..N en vez de 0..N−1, y compila perfecto). ✅ Poner el incremento DESPUÉS del nodo que consume (dentro del `:then` del cast puede ir como segundo statement), o inicializar en −1 si el orden no se puede cambiar.

308. 🔴 **`remove_function_graph` de una función que otro grafo LLAMA deja el nombre reservado**: el siguiente `add_function_graph` con ese nombre devuelve `<Nombre>_0` (§304 aplicado a este caso). ✅ Orden correcto para regenerar A→B (A llama a B): remover A, remover B, recrear B (+ params) escribir B, recrear A, escribir A. El call node del ConstructionScript hacia A **re-resuelve solo** al recompilar si el nombre coincide. Los `write_graph_dsl` intermedios compilan con error por la función faltante — el write queda igual; el error se va al recrearla.

309. 🔴 **La orientación de la V del UV no se resuelve teorizando: §302 (V invertida) NO se cumplió en el annulus del túnel.** La configuración correcta la fijó el ojo de Beltrán (dos veces mi "corrección" teórica rompió las proporciones: *"agujero demasiado pequeño"*). Regla: ante una magnitud radial/longitudinal que venga del UV, **un solo cambio por pasada, con el autor mirando el viewport** — su juicio es la medición (ver [[verificar-estado-estable-no-spawn]] y `beltran-autora-mirando`).

311. 🔴🔴 **Re-crear un componente con el MISMO NOMBRE que uno borrado deja un registro fantasma en las instancias ya colocadas que PISA el template del CDO.** Caso medido (Backdrop del túnel): el componente se borró y se volvió a crear con mesh+rotación en el CDO; la instancia del nivel lo mostró con `staticMesh None` y `relativeRotation 0` — el disco quedó acostado e invisible. Escribir la rotación directo en el componente de la instancia tampoco sobrevive al `load_level`. ✅ **El fix: RENOMBRAR el componente** (`Backdrop` → `Backplate`): un nombre nuevo no tiene registro viejo y el template aplica limpio. (Es "lo de la instancia le gana al Blueprint" a nivel de COMPONENTE.) Y lo que el CS escribe cada corrida (location/scale/params) es inmune al problema — otra razón para que el CS sea el dueño de la configuración.

310. 💡 **Perillas de look en una estación de galería: escribirlas en la INSTANCIA del nivel, no (solo) en el CDO.** La instancia spawneada congela los valores del momento del spawn (es el §"instance-editable nace en cero" en su forma de galería): cambiar el CDO + compilar no la toca. El bucle que funciona: `set_properties` a la instancia → `load_level` (re-corre el Construction Script) → captura. Y un cambio de CDO sin `compile_blueprint` no llega ni a las instancias nuevas.

## Cosecha 2026-09-07 (3ª tanda) — el giro del campo de puntos (BP_VoidField_SC)

312. 🔴🔴 **Los nodos `Sine`/`Cosine` de MATERIAL toman su entrada en VUELTAS, no en radianes ni en grados** (tienen una propiedad `Period`, default **1**). Bug real que vivio tres semanas sin que nadie lo viera: `BP_VoidField_SC` sumaba `+137` y `+251` grados al `DriftAngle` de las capas 1 y 2 para separarlas — pero 137 y 251 son **vueltas enteras**, o sea `frac = 0`, o sea **el mismo angulo que la capa 0**. Las tres capas derivaban en la misma direccion y el paralaje direccional nunca existio. El material compilaba, la animacion se veia, y el defecto solo se detecta preguntandose *"¿en que unidad esta esta entrada?"*. ✅ Igual vale para `RotateAboutAxis`: su `RotationAngle` tambien es en vueltas — si la perilla del autor esta en grados/seg, la conversion (`/360`) va explicita en el grafo.

313. 💡 **Girar un campo procedural es estrictamente mejor que trasladarlo**, cuando el patron se calcula sobre una direccion (`normalize(LocalPosition)`) o sobre una posicion de mundo. Una traslacion `pos + dir*Time*v` crece **sin cota**: a sesiones largas la precision float se degrada y el patron empieza a temblar. Una rotacion (`RotateAboutAxis` con pivote en el origen) es acotada por construccion — `Time` crece pero el resultado da vueltas. Receta: `RotateAboutAxis` devuelve el **desplazamiento** (`R·p − p`), asi que hay que **volver a sumarle `p`** para obtener `R·p`; olvidarse de esa suma da un campo que colapsa al origen.

314. 💡 **El orden barato para escribir un grafo DSL con variables categorizadas: escribir PRIMERO, categorizar DESPUES.** Por §306 los getters se namespacean por categoria, asi que si categorizas antes tenes que averiguar el nombre exacto (`Variables|E-Giro|GetSpinSpeed0`) con `find_node_types`. Si escribis el DSL con las variables todavia sin categoria, `Variables|Default|Get<Nombre>` funciona, y el `set_variable_category` posterior **no rompe los nodos ya creados**. Un round-trip menos por variable.

315. 💡 **`set_variable_instance_editable(v, false)` es la forma NO destructiva de retirar una perilla: la esconde del panel del actor Y del de Class Defaults, sin tocar el grafo.** Dos consecuencias practicas medidas (2026-09-07, `BP_Ganzfeld_SC` al quedarse solo con el modo fluido): (a) despues de eso **`ObjectTools.set_properties` sobre la instancia falla** con *"the following properties could not be set: X"* — se pierde `CPF_Edit`, asi que ese error es el **test positivo** de que quedo oculta, y tambien el aviso de que para cambiarla hay que rehabilitarla; (b) al ocultar una variable de eleccion (un `Mode`), hay que **fijar el default del CDO ANTES de compilar** y revisar las instancias colocadas: las que no tenian override adoptan el nuevo valor al recompilar (bien), pero una con override propio queda clavada en una opcion que ya no existe y sin panel para arreglarla.

316. 🔴🔴 **Cuando Beltran dice "la perilla X no hace nada", NO se verifica: se RASTREA.** (2026-09-07, `M_Ganzfeld_SC`.) El reflejo de montar un A/B con capturas para confirmar el sintoma costo una tanda entera y ~130k tokens **para llegar a lo que el ya habia dicho**; el diagnostico real salio de **seis `get_expression_inputs`** recorriendo la cadena hacia atras desde `MP_EmissiveColor`. Su reporte sobre su propio proyecto **es el dato**, no la hipotesis a testear (es `nota-en-negativo-provoca-rebuilds` y "la memoria de Beltran le gana a mis notas", aplicados a un sintoma). ✅ El bucle correcto ante "X no hace nada": (1) confirmar que el parametro esta en `MaterialInstanceTools.list_parameters` — si esta, COMPILA y llega a la salida por ALGUNA rama; (2) caminar la cadena viva hacia atras desde la salida y ver **quien alimenta de verdad** ese nodo. La trampa concreta: un parametro puede estar vivo alimentando una rama VIEJA y muerta mientras la rama nueva usa **otro parametro** (aca la escala del fluido la manejaba `VeilScale`, y el tiempo una `Constant`) — asi que "el parametro compila" NO prueba que este haciendo lo que dice su nombre.

## Cosecha 2026-09-07 (4a tanda) — el fluido del Ganzfeld: 6 reconstrucciones del subsistema equivocado

316b. 🔴🔴🔴 **Si N intentos DISTINTOS dan el MISMO sintoma, no estas tocando lo que se renderiza.** Caso: seis construcciones completamente diferentes del modo fluido (traslacion, vaiven, rotacion, marcha radial, crossfade de cortes, simplex 4D) y Beltran reportando *"sigue igual"* cada vez. No era mala suerte ni seis errores independientes: el pulso venia del **Veil**, una cadena aparte que multiplica el emisivo y que yo nunca habia mirado, con `VeilAmount = 3,13` tapando todo lo demas. ✅ **Regla de corte dura: al SEGUNDO "sigue igual", parar de construir y preguntarse "¿QUIEN MAS escribe a este output?"** — se responde recorriendo la cadena hacia atras desde `MP_EmissiveColor` y mirando **todos** los terminos, no solo el que uno cree que manda. Cuesta ~6 llamadas; me costo una jornada entera saltearlo.

317. 🔴🔴 **Antes de escribir matematica de shader propia, BUSCAR la implementacion canonica.** Escribi a mano un ruido de VALOR 4D y pulsaba. La razon estaba publicada y es tajante: el ruido de **gradiente** tiene *"enhanced smoothness and temporal stability, making it preferable for animations"*, mientras que el de **valor** deja *"the lattice quite obvious"*. La solucion canonica (**simplex 4D de Ashima Arts / Stefan Gustavson, `webgl-noise`, MIT**) existe desde hace 15 años, es la que usa todo el mundo, y ademas es **mas barata**: 5 esquinas contra las 16 del enrejado cubico. Lo dijo Beltran y tenia razon: *"busca en internet, te aseguro que esta la respuesta"*. 💡 Corolario: **un crossfade entre dos campos de ruido INDEPENDIENTES no deforma, DISUELVE** — se ve como "cambio el seed". El 4D de verdad interpola valores del MISMO enrejado; por eso hierve.

318. 🔴 **El alpha de un `lerp` NO es una perilla de intensidad.** `lerp(base, efecto, Amount)` solo se comporta en 0–1: por encima **extrapola** y el termino se come la imagen (aca `VeilAmount = 3,13` dominaba todo el material). Y el autor no elige mal el numero — elige alto **porque la perilla no responde como espera**. ✅ O se acota con `saturate` en el alpha, o —mejor— se expresa como ganancia (`base + efecto × Amount`), que es monotona y sin techo artificial. Sintoma tipico: una perilla autorada muy por fuera de su rango nominal es la **prueba** de que esta mal diseñada, no de que el usuario se equivoco.

319. 💡 **Un patron calculado sobre `normalize(LocalPosition)` gasta las TRES dimensiones del ruido y no deja ninguna para el tiempo.** Es la razon de fondo por la que "animar como el Noise TOP de TouchDesigner" no se traslada a un cascaron: alla el plano usa `(u,v)` y **deja Z libre**. Sobre una esfera, todo movimiento del punto de muestreo es **tangencial** (traslacion, vaiven, rotacion → se ve ir hacia un lado) salvo el **radial**, que es la normal de la superficie pero **es tambien la escala tangencial** (marchar por el radio achica las manchas). Y no se arregla con una carta 2D: una esfera no admite una sin costura ni polos. ✅ **Escala fija + evolucion en el lugar + sin direccion ⇒ hace falta ruido 4D, no es opcional.**

320. 🔴🔴 **Agregar un COMPONENTE nuevo a un BP con instancias colocadas muerde TRES veces, y las tres en silencio** (2026-09-07, el `Cone` de `BP_ShadowStudy_SC`): (1) la instancia lo recibe con **defaults de fabrica** — sin malla ni material aunque el template los tenga (§164) → escribirselos tambien a la instancia; (2) `add_component` por MCP lo crea con **`bVisible = false`** → todo verificado perfecto (malla, MID, transform, attach) y no se dibuja nada → prenderlo en instancia Y template; (3) si el material venia de otro BP, ese BP **neutralizaba terminos del material madre** que ahora nadie neutraliza (aca: `LaserFloorZ` default 0 rebano la sombra en horizontal "como un piso blanco"; tambien Smoke/Wobble) → leer las funciones Apply* del BP viejo y empujar los neutralizadores explicitos. 💡 El orden de diagnostico que funciono: attach → props del componente → **valores efectivos del MID** (`scalarParameterValues`/`vectorParameterValues` del `MID_*` de la instancia — es la verdad del render y una sola llamada).

321. 🔴🔴 **`CaptureViewport` puede devolver un FRAME VIEJO sin avisar. Dos capturas seguidas con bytes identicos = captura stale, NO es un resultado.**
    2026-09-08, midiendo la deformacion del Ganzfeld: tras una tanda de capturas correctas en el mismo encuadre, tres llamadas consecutivas devolvieron **exactamente el mismo PNG negro**. Coincidio con que Beltran estaba operando el editor en paralelo. Casi lo interpreto como "la esfera desaparecio" y salgo a debuggear un bug que no existia.
    ✅ **Control barato: hashear el base64 (o comparar los primeros 200 chars) entre capturas.** Si no cambio, la captura fallo — reintentar o cambiar de metodo, nunca sacar conclusiones. Y si el editor lo esta usando un humano, asumir que el viewport no es un instrumento confiable en ese momento.

322. 🔴 **Antes de usar un campo del material como HEIGHTMAP, calcula su RANGO REAL — casi nunca es [0,1].**
    Caso `M_Ganzfeld_SC`: el campo del veil parecia ir de 0 a 1 (sale de un `saturate`), pero aguas arriba los pesos crudos son `0.3 + 0.7*saturate(...)`, y al normalizarlos el maximo queda acotado en [1/3, 0.625] → **el campo final vive en [0,5625 , 1], con un piso de 0,5625**. Usado como desplazamiento eso es **56% de inflado constante y solo 44% de ola**: el sintoma que reporto Beltran fue *"en vez de deformar, agranda la esfera"*.
    ✅ **La receta**: (a) derivar min/max propagando las constantes del shader hacia adelante, (b) recentrar en el punto medio y escalar por 1/(rango), de modo que el parametro de amplitud signifique **pico-a-pico** y la media sea 0. Un campo con media ≠ 0 metido en un WPO radial **siempre** se lee como cambio de tamaño, no como relieve.
    🚩 Generaliza a cualquier `Noise`/`Custom` cuyo output se "ve" en [0,1] por venir de un saturate: el saturate acota, no normaliza.

323. 💡 **Para ver como resolvio algo OTRO material del proyecto, `MaterialInstanceTools.list_parameters(<el material>)` cuesta una llamada y suele alcanzar.** Los nombres de parametro delatan la tecnica: en `M_CloudPlane_SC` aparecieron `SlopeShade` y `ValleyColor`, que confirmaron que el oceano esculpe **por pendiente** y no por altura — justo lo contrario de lo que yo iba a construir. Mucho mas barato que `get_expressions` (30k chars) y mas confiable que recordar.

324. 🔴🔴 **Un material COMPARTIDO cuyos parametros el consumidor "apaga a mano" es una bomba de tiempo: la proxima perilla que se le agregue al material lo rompe, en silencio.**
    2026-09-08: el aro fijo de `BP_RingTunnel_SC` usaba `M_Ganzfeld_SC` (el material de la esfera) y su `Build` le apagaba `BreathAmount` y `VeilAmount` uno por uno. Al agregarle `WaveAmount` (WPO de olas) al Ganzfeld, **el aro empezo a deformarse** — el consumidor no tenia forma de enterarse de que existia una perilla nueva. Beltran lo diagnostico de una mirada: *"al tomar el de la esfera se deforma"*.
    ✅ **La regla: si un objeto solo necesita UNA PARTE de un material, necesita su propio material** — no una lista de apagados que hay que mantener sincronizada a mano. El reemplazo (`M_RingFrame_SC`) resulto ser **un solo parametro conectado al emisivo**.
    💡 **Y el olor que lo delata antes de que rompa:** si el consumidor empuja el MISMO valor a varios parametros para neutralizar un efecto (aca `FrameColor` iba a `ColorTop`, `ColorBottom` **y** `ColorMid` para que la mezcla diera plano), esta pagando un shader entero para no usarlo. Ahi hay un material propio esperando, y suele ser 10x mas barato.

325. 🔴🔴 **Cambiar una propiedad de COMPONENTE en el CDO no llega a los actores YA COLOCADOS en el nivel.**
    2026-09-08, `BP_ShadowStudy_SC`: se puso `TranslucencySortPriority = 100` en el componente `Sphere` del CDO, se compilo, y el cono seguia tapando la esfera. El actor del nivel tenia **`translucencySortPriority = 0`**: sus componentes estan **serializados en el mapa**, y reescribir la plantilla no los actualiza.
    ✅ **Al cambiar una propiedad de componente por MCP hay que escribirla en las DOS partes**: el CDO (`<BP>.<BP>_C:<Comp>_GEN_VARIABLE`, para los actores futuros) **y cada instancia** (`<mapa>:PersistentLevel.<Actor>.<Comp>`, para los que ya estan).
    💡 Y la verificacion barata es leer **la instancia**, nunca el CDO: `get_properties` sobre `.../<Actor>.<Comp>`. Es el mismo principio que [[instance-editable-nace-en-cero]] pero para COMPONENTES, no para variables — y muerde igual de callado.

326. 🚩 **"Que se vea por encima" solo se resuelve con `TranslucencySortPriority` si los DOS objetos son translucidos.**
    Si uno es opaco, esa propiedad **no hace absolutamente nada** sobre el: los opacos se dibujan en su propia pasada y se ordenan por profundidad. Para que un objeto opaco participe de esa ordenacion hay que **moverlo a la pasada translucida**: `BLEND_Translucent` + `MP_Opacity` conectada a una **constante 1** (explicita, no confiar en el default del compilador) + la prioridad. Con opacidad 1 y unlit se ve identico; lo unico que se pierde es la escritura de profundidad.
    ⚠ Antes de eso, comprobar que el problema no sea mas simple: si el translucido tapa al opaco es porque **su geometria esta realmente delante**. Reordenar es la respuesta cuando eso es inevitable (aca el cono nace dentro de la esfera); si no, mover/recortar la geometria es mas barato y mas honesto.

322. 🔴🔴 **Un selector de "modo" hecho con `Step` + `Lerp` sobre un parametro Scalar EVALUA TODOS LOS MODOS en cada pixel.** Un `lerp` no es una rama: el GPU calcula las dos entradas y despues mezcla. En `M_Ganzfeld_SC` (2026-09-15) eso significaba pagar los modos 0 y 1 — con 10 nodos `Noise` repartidos entre los tres — en el 100% de la pantalla, por dos ojos, cuando desde el 2026-09-07 la obra **solo usa el modo 2**. Medido en device: **24–27 ms de un presupuesto de 13,9 ms, con el cascaron como UNICO actor de la estacion**. ✅ La cura barata y reversible es **reconectar la salida del modo vivo directo al consumidor** (una sola `connect_expressions`): los otros modos quedan enteros en el grafo pero sin referencias, y el compilador los elimina. La cura definitiva es un **`StaticSwitchParameter`**, que es lo unico que genera ramas de verdad. 🚩 **La forma general:** ante un material caro, antes de optimizar el efecto que se ve, buscar **lo que se calcula y se descarta**. Y `list_parameters` lo canta de una: si el selector figura como `Scalar` y no como `StaticSwitch`, estan corriendo todos.

323. ⚠ **El nodo `Step` de UE tiene los pines `X` e `Y` y calcula `X >= Y`** — el umbral es **`Y`**, al reves de lo que sugiere el `step(edge, x)` de HLSL, donde el umbral es el PRIMER argumento. Leer mal esto invierte la rama que creias estar eligiendo. Se verifica por la estructura del grafo (que rama cuelga de que `Lerp`), no por la firma del nodo.


### ➕ Cosecha 2026-09-17 — la respiración portable en la galería

327. 🔴🔴 **Una función de ACTOR llamada sobre `self` con argumentos POSICIONALES pierde los literales en silencio.** `(Actor|GetComponentsbyTag "/Script/Engine.StaticMeshComponent" "TUNRING")` quedó con `ComponentClass = ActorComponent` y `Tag = None`: el primer posicional se lo comió el pin `self` y el resto se corrió. **Compiló en verde con `warnings_as_errors`** y el bucle que empujaba parámetros a los 16 anillos del túnel no iteraba nada. Se detectó porque el `read_graph_dsl` ya no mostraba `"TUNRING"` (el read SÍ refleja los valores reales de los pines: si un literal no aparece, no está).
    ✅ Arreglo: `set_pin_value` en los dos pines y **leer el nodo con `get_node_infos`**. Regla: después de escribir una llamada de actor con literales, **buscar esos literales en el read**; si falta uno, es esto. Con target explícito (`(Actor|GetComponentByClass _pawn "...")`) no pasa.

328. 🔴 **`CaptureViewport` captura el viewport ACTIVO aunque sea ORTOGRÁFICO; `captureTransform` no fuerza perspectiva.** Tres capturas "antes/durante/después" salieron con el mismo MD5 porque todas eran la vista Front del editor, que ni miraba el objeto. **Control positivo antes de comparar:** si dos capturas de estados distintos pesan exactamente lo mismo, el instrumento no está midiendo. Para verificar un enlace MPC → material sin visor alcanzó con otra evidencia: **tras `load_level` el motor recompila los shaders del nivel y un material roto deja `Failed to compile Material ...` en el log**; si no aparece, el `CollectionParameter` resolvió.

329. 🔴 **Nunca modular en caliente un `Speed` que el shader usa como `Time·Speed`**: con `Time` en cientos de segundos, cualquier cambio de `Speed` desplaza la fase entera (saltos). La velocidad se modula **integrando**: sumar a la fase `Speed·∫(ganancia)dt`. `MPC_Breath` publica `FlowIn = ∫max(S,0)dt` y `FlowOut = ∫max(−S,0)dt` justo para eso (receta en `M_RingTunnel_SC.BreathPhase`). Si la fase ya se acumula en el BP (`SweepPhase += Speed·DT`), multiplicar ahí es seguro.

330. ✅ **`MaterialExpressionCollectionParameter` por MCP**: `set_properties` de `collection` y después `parameterName` alcanza (el `parameterId` no se puede leer por `ObjectTools`, pero el motor lo resuelve). Y el nodo BP **`GetScalarParameterValue` de colección es IMPURO** (tiene pines exec): va encadenado en la ejecución, no colgado como getter.

331. 💡 **Para meter una señal global en N efectos sin acoplarlos a una clase: un MPC como bus.** Cada material multiplica su parámetro por `1 + max(S,0)·In + max(−S,0)·Out` con `In`/`Out` como parámetros del MID en 0 → **el material queda idéntico para todo el que no los prenda** (otras instancias, otras estaciones, la obra). Los BPs solo empujan las ganancias en el Construction Script; nadie tickea para eso. Solo lo que es geometría de CPU (una escala de componente, una fase acumulada) lee el MPC en su Tick.

332. 🔴🔴 **Un CLAMP duro sobre una señal de biofeedback se siente como un tope ("llegó muy duro a los máximos y mínimos").** La v1 de `BP_BreathManager_SC` publicaba `clamp(gain·y, −1, 1)`: con ganancia 1,5 una respiración normal chocaba contra ±1 y el parámetro quedaba plano hasta que la respiración volvía. Numéricamente "funcionaba"; en el cuerpo se sentía cortado. La esfera de Entering nunca tuvo tope porque su nivel sale de `x/(1+|x|)`.
    ✅ **Ganancia con techo suave**: `S = k·y / (1 + (k−1)·|y|)` (monótona, derivada continua, se acerca a ±1 sin meseta). Y en los consumidores, **nada de funciones por tramos con quiebre** (`max(S,0)·In + max(−S,0)·Out` cambia de pendiente en 0 si las ganancias son asimétricas): `1 + S·lerp(−Out, In, smoothstep((S+1)/2))`. Lo mismo para pisos: `max(x, 0)` → rampa cuadrática.
    🚩 Regla: **en todo lo que el cuerpo del usuario maneja, suavidad C1 como mínimo**. Un test que mide valores no lo detecta: hay que muestrear la curva en el tiempo y mirar si hay mesetas.

333. ⚠ **Agregar un parámetro a una función NO refresca los nodos que ya la llaman**: el compile falla con *"Could not find a pin for the parameter On of StepBreath"*. Salida: borrar la llamada y crearla de nuevo con `create_node` (sale con todos los pines), y recablear.

334. 🔴 **"Llega demasiado rápido al máximo" en una respiración LENTA = el band-pass adelanta el pico y las saturaciones encadenadas lo aplastan.** Un band-pass `EMA(0,4 s) − EMA(τ)` tiene adelanto de fase: con τ = 3 s y una respiración de 12-16 s el pico sale 1-2 s antes del final de la inhalación. Y dos saturaciones suaves en cadena (`x/(1+|x|)` y luego `k·y/(1+(k−1)|y|)`) equivalen a UNA sola con escala `GainK/k`: casi todo el recorrido queda pegado al máximo. Síntoma de Beltrán: *"deben demorarse más en llegar al máximo, acompañando mi movimiento"*.
    ✅ Para biofeedback que acompaña: τ de la base más largo (6 s), **una sola** curva que sea lineal en el rango normal y tenga codo suave al final (`x/(1+|x|³)^(1/3)`), y un **resorte críticamente amortiguado** en vez de FInterpTo en cascada (velocidad continua = "suavidad elegante" sin agregar más retardo). Verificar muestreando la curva con una respiración de prueba LENTA: el máximo tiene que caer al final de la media onda.

335. 💡 **Para que "la esfera crece" se VEA desde su centro, los puntos tienen que conservar su tamaño en el mundo.** Un patrón pintado por dirección (`normalize(LocalPosition)`) no cambia en pantalla si la esfera se escala alrededor del ojo. Escalar por WPO + dividir el radio del punto por la escala (`Size/m`) hace que los puntos se alejen/acerquen de verdad. Al achicar, limitar el crecimiento al margen de la celda y nunca por debajo del tamaño autorado, o vuelven los cortes y cambia el reposo.

336. 🔴🔴 **Una escala FIJA en cm no sirve para biofeedback: cada cuerpo mueve el sensor distinto.** La 3ª pasada del manager mapeaba `x = bp[cm]·1`. Con la respiración de Beltrán (~3 cm de band-pass) el codo de la curva se cruzaba en el primer tercio de la inhalación: *"no he terminado de inhalar y ya llegó rapidísimo al final"*. El análisis de los 14 usuarios ya lo había dicho ("inhala/exhala no generaliza con umbral global; normalizar por usuario") y el diseño lo ignoró.
    ✅ **Ganancia automática**: `AmpEMA = EMA(|bp|, 8 s)` mientras hay umbral, `x = bp / max(AmpEMA·π/2, piso)` (para una onda senoidal `media|x| = 2A/π`). Una respiración normal de cualquiera da `x ≈ ±1` justo al final; las profundas van más allá con codo suave. Medido: entrada de 3 cm / 10 s → primer ciclo satura mientras aprende, desde el tercero pico ±0,79 exactamente al final de la media onda.
    🚩 **Y la lección de método, que es la más cara:** la respiración de prueba de la 3ª pasada inyectaba `x` ya normalizado → **el PIE daba perfecto justo en la parte que fallaba en visor**. Un instrumento de prueba tiene que entrar por el MISMO camino que la señal real (ahora `FakeAmpCm` genera centímetros que pasan por la ganancia automática).

337. 🔴 **Envolver cada tool en `T()` NO alcanza: el cuerpo de `run()` también tiene que ir en try/except.** El 2026-09-18 un `p.get('name', '')` mío (el sandbox usa `_StrictDict`, que **no acepta default en `.get()`**) levantó `TypeError` DESPUÉS de que el script ya había hecho sus cambios → el plugin disparó el `Undo Execute tool script` de siempre. Las llamadas al MCP estaban todas protegidas; **el error fue de mi propia lógica Python**, que es justo lo que `T()` no cubre. ✅ La plantilla `scripts/safe_script.py` ahora envuelve el cuerpo de `run()` y devuelve el error como dato. Y en ese sandbox: `dict.get(k)` sí, `dict.get(k, default)` **no**.

338. 🔴🔴🔴 **Un pin de ejecución de ENTRADA acepta VARIAS conexiones: `connect_pins` sobre él SUMA, no reemplaza.** La regla "conectar a una entrada ya conectada la REEMPLAZA" vale para **pines de datos**. En los de exec, el que es de enlace único es el de **SALIDA**. El 2026-09-18, reordenando el Tick de `BP_ShadowStudy_SC`, conecté `Tick.then → StepBreath.execute` y `StepBreath.then → StepSweep.execute` dando por hecho que el enlace viejo se iba: quedó `Tick → StepBreath → StepSweep → GetScalarParameterValue → StepBreath → …`. **Reordenar una cadena de exec exige `break_pins` explícito de los enlaces viejos.**
- 🔴 **`read_graph_dsl` NO delata el ciclo: lo imprime LINEAL** (sigue un camino y corta). El grafo se leía perfecto. La única verificación válida es `get_node_infos` mirando **las dos puntas**: cada entrada de exec con **exactamente una** fuente, y el último nodo con su `then` vacío.
- 🔴 **Y `compile_blueprint` devuelve `true`**: un bucle infinito de exec compila sin chistar. No es un error de compilación, es de ejecución.
- **Síntoma en visor (lo que reportó Beltrán):** *"le pongo play y se queda pegado y crashea; en la compu se veía el visor pero pegado, y en las gafas no se veía nada"*. El hilo de juego queda atrapado en el bucle → el preview se congela y nunca llega un frame al visor.
- **Dónde está escrito:** `PIE: Error: Infinite loop detected. Blueprint: <BP> Function: <X>`. ⚠ **La función que nombra NO es la del bug**: es donde estaba el contador cuando saltó el guardia (acá dijo `Branch` y `GetScalarParameterValue`). Lo útil del mensaje es el **Blueprint**, no la función.
- ⚠ **Contá los nodos de exec ANTES de reordenar.** El Tick parecía de 2 llamadas; eran 3, porque `Rendering|Material|GetScalarParameterValue` (MPC) es **impuro** y vive en la cadena de ejecución.

339. 🔴🔴 **Una VELOCIDAD no se puede manejar con una función sin memoria de una señal de POSICIÓN.** La respiración publica `S` = dónde está la panza. Los mapeos de posición (tamaño, brillo, apertura) andan perfecto con `S`. Pero para las velocidades el manager integraba `FlowOut = ∫neg(S)dt`, y **al empezar a exhalar `S` está en su máximo POSITIVO**: la primera mitad de cada exhalación integraba cero y el efecto aparecía recién al final. Síntoma de Beltrán: *"recién se activa la velocidad rápida al llegar al final de la exhalación; debería ir aumentando según voy exhalando"*.
- 🔑 **No hay `f(S)` que lo arregle.** Si `f` es monótona y `f(0) = 0` (reposo neutro), entonces `f(S) < 0` para todo `S > 0`, que es justo la primera mitad de la exhalación. Cambiar la curva, el suavizado o las ganancias **no puede** resolverlo: una función sin estado no distingue "arriba y subiendo" de "arriba y empezando a bajar".
- ✅ **La salida es estado: medir el PROGRESO, no la posición.** Seguir el pico y el valle (`SPeak = max(S, decaimiento lento)`, `SValley` simétrico) y publicar `u = clamp((SPeak − S)/max(SPeak − SValley, ε))` = *cuánto llevo exhalado*, que vale 0 arriba, 1 abajo y sube lineal en el medio, se congela en reposo y **se normaliza solo a la amplitud real de cada respiración**. Ver `BP_BreathManager_SC` §2026-09-18.
- 🚩 **La regla general:** antes de mapear algo a la respiración, preguntarse si lo que se mueve es una POSICIÓN (va con `S`) o una VELOCIDAD (va con el progreso). Mezclarlas es el defecto, no los números.

340. ⚠ **`remove_function_graph` + `add_function_graph` con el MISMO nombre en la misma tanda devuelve `Nombre_0`.** El nombre viejo sigue reservado hasta que el Blueprint recompila. Síntoma: todo lo que sigue falla con *"X is not valid EdGraph for property 'graph'"* porque el grafo real se llama `X_0`. ✅ **`compile_blueprint` entre el remove y el add**, y después **confirmar el nombre real con `list_graphs`** en vez de asumirlo. Y ojo: si ya habías borrado el nodo que llamaba a esa función, el grafo que la llamaba **queda con la cadena de ejecución cortada** — acá `Publish` se quedó sin `PushMPC` (o sea, el manager dejó de publicar al MPC) y solo se vio revisando la cadena, porque **compila igual**.

341. 🔴 **Un componente agregado al CDO llega SIN MALLA a los actores YA COLOCADOS** (§320 otra vez, 2026-09-18). Se le puso `staticMesh` al componente `Drop` del Blueprint y la instancia del nivel quedó con `staticMesh = None`: **la esfera no existía**, y lo que se veía (y lo que Beltrán estuvo ajustando media hora) era otra cosa — el bulto del campo de altura. ✅ Escribir `staticMesh` **también en la instancia**, y verificarlo leyendo la propiedad del componente del actor, no del CDO.
- 🚩 **La trampa de diagnóstico:** la captura mostraba una **semiesfera perfecta**, que es exactamente lo que se ve cuando una esfera está centrada en z = 0. Esa hipótesis era razonable **y era falsa**: no había esfera. Dos causas distintas daban la misma imagen. Cuando una imagen admite dos explicaciones, se **lee la propiedad**; no se elige la más probable.

342. 🔴 **Una perilla que el shader CLAMPEA por debajo de lo que el panel acepta miente en silencio, y el síntoma no se parece a la causa.** Tercer caso en el mismo material: `Steps` clampeado a 64 (todo lo de arriba no hace nada) y `GroupCount` clampeado a 8 mientras Beltrán lo tenía en **10**. Con 12 gotas y 8 grupos, `g = j mod 8` repartía **4 grupos de 2 y 4 de 1**, y los de 2 se funden por el smooth-min → su queja fue *"hay unos que se ven enormes y otros pequeños"*, que **suena a un problema de tamaños** (y `SizeVariation` ya estaba en 0, o sea que los radios eran idénticos). ✅ Al diagnosticar "se ve desparejo", **leer los `clamp` del shader y compararlos contra el valor real de la instancia ANTES de mirar la geometría**: es una llamada y descarta media hipótesis. Y al escribir un `clamp` nuevo, que el tope sea el límite REAL (acá `min(GroupCount, BlobCount)`), no un número redondo elegido a ojo.

343. 💡 **Con `Smoothness` mucho mayor que `BlobRadius`, el "tamaño" que se ve NO es el radio: lo fija el smooth-min.** En `GAL_7_MetaBlob` eran 26,9 contra 1,28 (×21). Consecuencia práctica: **el tamaño aparente de un blob depende de cuántos vecinos tiene cerca**, así que cualquier arreglo por gota (radios, `SizeVariation`) no toca el problema — hay que **igualar la configuración**: misma cantidad de gotas por racimo, equidistantes, y el ruido aplicado **al racimo entero** y no por gota, para que no fusione distinto en cada uno.

344. 🔴🔴 **Puede haber OTRA sesión de Claude manejando el MISMO editor.** El 2026-09-19, revisando `VR_Test.log` por otro motivo, aparecieron `add_variable`, `set_variable_category`, `add_function_graph`, `find_node_types` y `connect_expressions` **intercalados entre mis propias llamadas**, y `M_PulseField_SC` pasó a *"Failed to compile Material ... Default Material will be used in game"* sin que yo lo tocara. Es peligroso de verdad: además del riesgo obvio de pisarse, **un `execute_tool_script` que falla en la otra sesión dispara un Undo que saca una transacción de la pila GLOBAL — que puede ser la mía** (§"El Undo que borra el nivel"). ✅ `grep "Dispatching toolset tool" VR_Test.log | tail -20` distingue en segundos las llamadas propias de las ajenas; si aparecen ajenas, **avisar y parar de escribir** antes de seguir.

## Cosecha 2026-09-19 (anillos que se elevan + extracción del latido)

### 🔴🔴 `SetVisibility` desde el Construction Script NO apaga nada — el `bVisible` se graba como override de instancia
Construyendo los anillos de `BP_PulseField_SC`: el CS ponía `SetVisibility(false)` a los anillos muertos y **seguían visibles**. Causa: cuando el CS pone `bVisible = true` y el template del componente está en `false`, la diferencia **se graba como override de la instancia** y el editor la **reaplica DESPUÉS** del Construction Script. Los que alguna vez estuvieron vivos quedaban clavados en visible.
🚩 **La firma que lo delata**: la lógica fallaba con sobrepaso CHICO (u=1,07) y funcionaba con sobrepaso GRANDE (u=32) — porque solo los que alguna vez se vieron tenían el override.
✅ **Para apagar un componente desde el CS, usar la TRANSFORM, no la visibilidad**: `SetRelativeScale3D = 0`. Las transforms sí se reaplican en cada pasada. (Y dejar `bVisible = true` fijo en template e instancia.)
👉 Es otra cara de la regla #1 del proyecto: *lo de la instancia le gana al Blueprint*.

### 🔴 Un fundido a negro NO es un fundido (material unlit opaco)
Multiplicar el emisivo de un material **unlit + opaco** por un `Fade` no lo desvanece: lo lleva a **negro**. Contra un fondo claro, el objeto moribundo se lee como una banda NEGRA. Lo cazó Beltrán en el viewport. En un cielo/void oscuro el truco funciona; en cuanto el fondo se aclara, no. Si hace falta que desaparezca de verdad: translucidez (cara en Quest), o que lo coma la **niebla** (la salida elegida acá: `BP_FogSlab_SC`).

### 🔴 Colisión de nombres en DISPATCHERS, no solo en funciones
`Default|CallOnBeatPulse` en un BP nuevo se cableó al dispatcher **de `BP_Sensor_Soul`** (que estaba cargado en memoria) y el BP falló a compilar con *"This blueprint (self) is not a BP_Sensor_Soul_C, therefore 'Target' must have a connection"*. `find_node_types` filtrando por el nombre devuelve el **mismo string dos veces** — esa duplicación ES el aviso. **Arreglo: ponerle un nombre único al dispatcher** (`OnHeartBeat`).
⚠ Y no hay `remove_event_dispatcher` en el MCP: el dispatcher descartado hay que borrarlo a mano en el panel My Blueprint.

### 🔴🔴 `CallFunction|MiFuncion` con argumento POSICIONAL cae en el pin `self` — y a veces COMPILA Y MIENTE
Dos caras del MISMO bug, el mismo día:
1. **Ruidosa:** `(CallFunction|PulseHaptic (Variables|Z-Estado|GetRight))` → error *"Could not connect pin bRight to self"*. Se ve al toque.
2. 🔴 **SILENCIOSA, y costó tres pasadas de visor:** `(CallFunction|FindHand true)` y `(CallFunction|FindHand false)` **compilaron sin una sola queja** y el parámetro `bRight` quedó **desconectado en las dos llamadas**, o sea en su default `false`. Resultado: `HandR` y `HandL` apuntaban **los dos al mando IZQUIERDO**. La mecánica del latido "no existía" porque medía la mano equivocada (el usuario con el mando derecho en el pecho, el manager mirando el izquierdo apoyado en la pierna → `v = 117 cm` clavado).
✅ **Regla: para funciones PROPIAS, SIEMPRE `:Nombre valor`. Nunca posicional.** Es la misma familia que la trampa #4 del DSL (los literales string de funciones propias que se pierden en silencio).
🔎 **Cómo se caza en 1 llamada:** `ObjectTools.get_properties` sobre el actor **del mundo de PIE** (`/Game/UEDPIE_0_<Mapa>.<Mapa>:PersistentLevel.<Actor>`) pidiendo las refs cacheadas, y **comparar contra el BP que ya funciona**. Ahí las dos filas lado a lado gritan el bug. Los `PrintString` de geometría mostraban números "raros pero plausibles" y no lo delataban.

### ⚠ `Utilities|Array|MakeArray` solo expone UN pin
`(Utilities|Array|MakeArray a b c ...)` falla con *"received 6 positional arg(s) but has 1 data input pin(s)"* — el nodo es variádico en el editor pero el DSL no le agrega pines. Para sembrar un array de N: `Utilities|Array|Clear` + N × `Utilities|Array|Add`.

### ⚠ Nombres de tools que cuestan un round-trip (medidos acá)
`ObjectTools.set_properties` = **`values`** (string JSON), no `properties`. · `ObjectTools.get_properties` = **`instance`** + **`properties`**. · `MaterialInstanceTools.list_parameters` = **`material`**. · `AssetTools.save_assets` = **`asset_paths`** (strings, no refs). · `AssetTools.find_assets` = **`folder_path`** + **`name`**. · `BlueprintTools.add_event_dispatcher` = **`name`**. · `SceneTools.find_actors` exige `name`, `tag` **y** `collision_channels` aunque no se usen. · `mcp__unreal__call_tool` usa **`arguments`**, no `tool_args`.
Y las escribibles de material sobre componente llevan **`on` minúscula**: `Rendering|Material|Set{Scalar,Vector,Color}ParameterValueonMaterials` (el `read` las imprime con `On` mayúscula, que NO es escribible).


### 🔴🔴 Un vector de TRANSFORM escrito como JSON entra SOLO en su primera componente (2026-09-19)
`ObjectTools.set_properties(<rootComponent>, '{"relativeLocation":{"x":1,"y":2,"z":3}}')` **devuelve `true`** y el componente queda en **(1, 0, 0)**.
Síntoma con el que se encontró: se colocaron los 33 actores de una estación nueva y **los 8 slots quedaron apilados en el mismo punto**,
con la Y en 0 — o sea a 100 km del usuario — aunque la X estaba perfecta. Beltrán: *"los slot estan en cualquier lado, no frente al ususario"*.
✅ **Arreglo: el formato de TEXTO de Unreal.** `'{"relativeLocation":"(X=1.000000,Y=2.000000,Z=3.000000)"}'` escribe los tres.
⚠ **No generalizar**: el mismo JSON sobre un `FLinearColor` de una expresión de material sí escribió los 4 campos. Es el setter de transform
de `SceneComponent` el que se come el resto.
🔎 **Lo único que lo caza es releer.** Los dos caminos devuelven `true`. La regla de la casa (§"post-write read") aplica literal:
después de mover algo por MCP, leer la transform y **comparar contra lo pedido**, no confiar en el valor de retorno.

### 🔘 Un botón por hover+gatillo necesita que el gatillo BAJE dentro del hover (2026-09-19)
`BP_MenuButton` contaba el hold con `bTrigHeld` a secas: si el usuario ya venía **apretando el gatillo** y metía la mano en el hover, el botón se activaba solo. Beltrán: *"si tengo el trigger apretado y toco el botón, lo toma como apretado y se activa el avance"*.
...
## Cosecha 2026-09-21 (tarde) — la revisión de Beltrán del nivel V3 en el editor

345. 🔴🔴 **Un nivel DUPLICADO hereda los sublevels del original, y en el EDITOR se ven encima del nuevo.** `L_SoulCharger_V3` nació de duplicar `MapsV2/L_SoulCharger` y se trajo registrados los 6 sublevels de V2. En juego no cargan (`bInitiallyLoaded = false`, y `RoomLevels` de V3 = solo el Hall), pero **en el viewport están todos**. Beltrán reportó cuatro cosas que "V3 todavía tenía" y ninguna existía en V3: la esfera `BP_BreathOrb_SC`, los interiores de Entering y Recognizing, y el Niagara `BP_NeuralWeb_SC` de Loving. **Todas vivían en los sublevels de V2**. Y taparon una verificación mía: la captura de la vista previa de color salió azul porque el cilindro de Entering de V2 envolvía la cámara.
   - 🔎 **Diagnóstico en una llamada:** `SceneTools.find_actors('')` agrupado por paquete (`r.split('.')[0].split('/')[-1]`). Salió `L_Entering_SC: 8 · L_Recognizing_SC: 12 · L_Loving_SC: 5 · L_Attracting_SC: 37 · L_Surrounding_SC: 5` al lado de los 130 de V3.
   - ❌ `bShouldBeLoaded`/`bShouldBeVisible = false` en el `LevelStreamingDynamic_N` **no cambian nada en el editor** (y en juego se pisan, §107). El MCP **no puede sacar un sublevel** del mundo, igual que no puede registrarlo.
   - ✅ **Se saca desde el panel Levels**: *Window → Levels*, seleccionar los sublevels ajenos, clic derecho → *Remove Selected*, guardar. No toca los archivos de los sublevels ni el nivel original.
   - 👉 **Regla: al duplicar un persistente para hacer un nivel nuevo, desenganchar en el momento los sublevels que no le pertenecen**, y antes de creerle a una captura del editor, contar los actores por paquete.

346. 🔴🔴 **`computer-use.open_application` sobre el ejecutable de Unreal ABRE OTRO EDITOR, no enfoca el que hay.** El 2026-09-21 levantó un segundo `UnrealEditor.exe` (el Project Browser, 3,4 GB). Beltrán: *"¿por qué estás abriendo otro editor? ya tengo el proyecto"*. Se cerró con `CloseMainWindow` sobre ese PID (filtrado por `StartTime`, para no tocar el de la sesión). Además, en el monitor secundario **los clics cayeron corridos** (apuntando a *Window* se abrió *Actor*). 👉 **No manejar la UI del editor con computer-use.** Si un paso solo se puede hacer desde la UI (sacar un sublevel, un diálogo modal), se le pide a Beltrán con las instrucciones exactas.

347. **`MaterialInstanceTools` solo acepta `MaterialInstanceConstant`**: sobre un MID (el que crea `SetColorParameterValueOnMaterials`) falla con *"is not valid MaterialInstanceConstant"*. ✅ Para LEER un MID: `overrideMaterials` del componente → el MID → `ObjectTools.get_properties(MID, ['vectorParameterValues', 'scalarParameterValues'])` (cada entrada trae `parameterInfo.name` + `parameterValue`). Es la forma de verificar lo que realmente recibió el material, que es lo que importa.

348. **Con PIE corriendo:** `SceneTools.remove_from_scene` se niega (*"Cannot remove actors while PIE is active"*) y las variables de estado **no instance-editable** (p. ej. `Phase` del secuenciador) **no se pueden escribir** con `set_properties`. Lo que **sí** se puede: mover actores de PIE por su root component (así se probó que el metaball sigue a la fila). Para cerrar la intro de Attracting sin manos: `bAutoTest = true` en `Director_Story` (fuerza el panel → `TickIntro` completa la intro sola) — **y devolverlo a su valor al terminar**.

349. 🔴 **DSL: una función NATIVA sobre `self` con un primer argumento posicional se cablea al pin `self`.** `(Transformation|SetActorLocation _to false false)` → *"Could not connect pin ReturnValue to self"*. ✅ **Keywords**: `(Transformation|SetActorLocation :NewLocation _to :bSweep false :bTeleport false)`. Es la misma familia que la regla de las funciones propias (trampa 1 de `BP_StageShell_SC`): **cualquier llamada sobre self con argumentos va con keywords**. Y el read del DSL puede mostrar una llamada propia con el nombre de OTRO Blueprint (`Class|BPSoulHUD|ApplyPulse` para el `ApplyPulse` del metaball): es un artefacto de la lectura; lo que manda es el pin `self` en `get_node_infos` (`Self Object Reference` = es la propia).

350. 💡 **Un proxy de raymarch NO uniforme sin deformar el SDF:** multiplicar `RayOrigin` y `RayDir` (ya en espacio local) por el estiramiento por eje `ProxyStretch = 1 − ProxyTrim`. El SDF sigue isótropo y la caja puede ser chata. Es fill-rate puro en Quest: la fila de 8 gotas del secuenciador con cubo uniforme habría pedido ~2,5 m de caja, **media vista con 64 × 8 evaluaciones por píxel**; recortada queda en 45 × 255 × 45 cm. Y con `twoSided = false` **la cámara tiene que quedar FUERA de la caja** (adentro las caras traseras se descartan y el efecto desaparece), otro motivo para recortarla.

351. 🔴 **Un proxy translúcido hace el depth-test en su cara DELANTERA, no en el punto de impacto del rayo.** Lo opaco que quede **dentro** de la caja queda pintado por encima aunque la gota esté detrás. 👉 La caja entera va **detrás** (o delante) de la geometría con la que convive; la fila del secuenciador va 17 cm detrás de los slots por eso.

352. 💡 **Perillas nuevas con el 0 como valor neutro esquivan la trampa de "nace en cero en la instancia"** (`instance-editable-nace-en-cero`). `ProxyTrim` (0 = cubo entero) y `PreviewStage` (0 = apagado) nacieron en 0 en las instancias ya colocadas —las 3 de V3 y las de la galería— y ese es justo el valor que no cambia nada: no hubo que sembrar ninguna. Al diseñar una perilla nueva sobre un Blueprint ya colocado, **elegir la semántica para que 0 signifique "como antes"**.

353. 🔴🔴 **El Construction Script NO apaga la colisión de un actor YA COLOCADO — ni en el editor ni en juego.** Un actor del mapa no re-ejecuta su CS en runtime, y `set_actor_transform` / `set_properties` por MCP tampoco lo disparan; encima el `BodyInstance` serializado de la instancia le gana al template del Blueprint (escribirlo con `set_properties` devuelve `true` y **no entra**). ✅ Para que una instancia colocada quede sin colisión: **`SetActorEnableCollision(false)` en el `BeginPlay`** (un nodo, cubre todos los componentes). Y para el editor, tocar cualquier perilla del actor re-corre el CS. Caso: el proxy del metaball del secuenciador tapaba el beam de Attracting (2026-09-23).

354. 🔴 **`get_properties(componente, ['bodyInstance'])` MIENTE: devuelve el valor serializado, no el efectivo.** Después de apagar la colisión en runtime siguió informando `QueryAndPhysics` **también dentro de PIE**. El instrumento válido es **`SceneTools.trace_world`**, con dos precauciones: (a) validarlo primero contra algo que se sabe sin colisión (debe devolver `null`); (b) usar un **test de identidad** — mover el actor sospechoso 10 cm y ver si el punto de impacto se mueve con él. Y ojo con leer un impacto "más lejos" como "no pegó": un proxy con las caras invertidas devuelve la **cara de salida**, que parece otro objeto.

355. 🔴 **Para repartir N cosas (fases, tamaños, ritmos) usar el ÁNGULO ÁUREO, no un hash.** `frac(sin(j·k))` con 8 muestras agrupa por azar: en la fila de gotas del secuenciador unas vecinas quedaron en fase (siempre unidas) y las puntas en contrafase (nunca). `j · 2.39996` (137,5°) reparte parejo, es determinista y no se repite. 🔴 En VR además es obligatorio que la fuente sea el **índice**, nunca el píxel ni el frame: cualquier aleatoriedad por píxel da valores distintos en cada ojo y rompe la estereoscopía.

356. 💡 **Un promedio ponderado entre N elementos DILUYE lo que se quiere destacar.** El tinte por pulso del metaball se calculaba como `Σ w·p / Σ w` sobre las 8 gotas: con el kernel del smooth-min, las vecinas apagadas sumaban ~3 al divisor y el color llegaba al 15% aun sobre la gota encendida (*"elegí un azul y no se ve tan azul"*). ✅ **Máximo ponderado** (`max(w_j · p_j)`): vale 1 sobre la que pulsa y conserva el degradé hacia las vecinas. Corolario general: si el efecto es "esta de acá", el operador es `max`, no la media.

357. ⚠ **Una animación de adorno no puede tener dirección si compite con una mecánica que ya avanza.** La onda que mueve la zona de unión de las gotas barría la fila en un sentido y se leía como un segundo playhead. ✅ Dos ondas **contrapropagantes** con frecuencias espaciales y temporales inconmensurables: el patrón hierve sin barrer. Misma razón para que la velocidad de cada elemento no dependa de su posición en la fila (eso también es un barrido, aunque sutil).

358. 🔴🔴 **Una vista previa hecha en el Construction Script APARECE EN PIE y BLOQUEA EL RAYO.** El CS también corre al dar play, así que la previa se dibuja **encima** del objeto real (*"muestra esferas dobles"*) y, como un `AddComponent|Common|AddStaticMeshComponent` **nace con colisión**, se come el line-trace del beam (*"no me deja agarrarlas"*). ✅ En el mismo bucle que la crea: `SetCollisionEnabled(comp, NoCollision)` + `SetHiddenInGame(comp, true)` — sigue viva en el viewport y desaparece al dar play. ⚠ Esas dos líneas van en la función de la **previa**, NO en la función de look compartida con el objeto real, o lo dejan invisible también (2026-09-23, `BP_OrbDirector_SC`).

359. 🔴 **Los marcadores de debug colocados en el nivel COLISIONAN, y esconderlos no alcanza.** La esfera verde de `BP_Anchor` (`/Engine/BasicShapes/Sphere` a 0,18 = **radio 9 cm**) tapaba el beam justo donde estaba el objeto agarrable. `SetVisibility(false)` no cambia nada para un trace: hay que apagar la **colisión**, y hacerlo **siempre**, no atado a la perilla que los muestra. 💡 **El síntoma "no puedo agarrar / algo tapa el rayo" se diagnostica con `trace_world` leyendo la DISTANCIA**: un trace de 2 m que cruzaba el ancla devolvió **91 cm** sobre un inicio a 100 cm → el bulto empezaba 9 cm antes del centro, que es exactamente ese radio. La distancia identifica al culpable por tamaño.

360. 🔴🔴 **Antes de empujarle un valor a un objeto, buscá QUIÉN MÁS lo escribe en el Tick.** El director le seteaba la escala al `Body` de la esfera y en pantalla salían todas iguales: `UpdateVisual` la reescribía cada frame desde `BaseScale`, que `BeginPlay` había capturado de la escala del componente (idéntica en todas). Un push de una sola vez **no sobrevive al primer frame** frente a un consumidor por frame. ✅ La palanca no es el valor final sino **la variable de la que ese Tick lo deriva**. Corolario del mismo caso: `Setup` (que llama el orquestador tras spawnear) corre **después** de `BeginPlay` — se comprueba solo cuando una variable se escribe en los dos y gana la del segundo.

361. 🔴 **Un parámetro ABSOLUTO no tiene valor neutro: hay que darle un flag de "no aplicar".** El tamaño de anclaje de las esferas se pidió *"el mismo para todas, independiente de si en el espacio hay unas más grandes que otras"*. Como factor (×0,75) conserva la variedad pero **no cumple el pedido**; como absoluto sí, pero entonces 0 no significa "como antes" sino "invisible", y la perilla nace en 0 en toda instancia ya colocada (§`instance-editable-nace-en-cero`). ✅ Un bool interno (`bUseAnchor`, false por defecto) que sólo enciende quien configura. Regla al diseñar: si la semántica no admite un neutro, **el neutro va en un flag aparte**, no en el valor.

362. 🔴🔴 **Si el Blueprint necesita saber dónde está algo que dibuja el SHADER, mové el cálculo al Blueprint — no lo repliques.** Las gotas del metaball flotaban por WPO en el HLSL, así que el BP no sabía el centro real y la esfera anclada no podía seguirlo. Replicar el seno en BP obliga a que el `Time` del material y el del juego coincidan; si no coinciden, el objeto deriva **en contrafase** con lo que debe seguir — peor que no moverse. ✅ Se movió la deriva a `GetBlobOffset()`, `PushCenters` la suma al centro que empuja y **le manda `FloatAmount = 0` al material** para que no la aplique dos veces. Una sola fuente, coincidencia exacta por construcción. ⚠ Cuando sólo se puede replicar (el `fac` de tamaño del mismo shader), replicá el **caso más conservador** (restarle la amplitud de la onda en vez de seguirla) para que la garantía se cumpla siempre — a costa de que el efecto se lea más tímido de lo que dice la perilla.

363. 🔴 **Al mudar un cálculo del shader al Blueprint hay que neutralizar el push del knob en TODOS los caminos, no sólo en el del Tick.** La deriva de las gotas se movió al BP y `PushCenters` manda `FloatAmount = 0` al material — pero `ApplyAll` (Construction Script) seguía empujando el valor autorado **después**, así que en el editor el shader volvía a flotar las gotas encima de unos centros que ya traían la deriva: doble offset, y el adorno que debía ir al centro quedaba descolocado. ✅ El `= 0` va en la **última** función de la cadena del CS. 💡 Y se verifica leyendo el **MID del componente**, no el Blueprint: `get_properties(MID, ['scalarParameterValues'])` decía `FloatAmount 1.234` mientras la variable decía otra cosa. El MID es el valor efectivo.

364. 💡 **Para comprobar que dos cosas están en el mismo lugar, compáralas en el MISMO espacio y con números, no a ojo.** Los centros de las gotas viven en el MID como `C0..C7` (locales al actor) y el componente del núcleo tiene `relativeLocation` (también local): si coinciden dígito a dígito, están concentricos. Salió `C3 = (18.15758, -11.35255, -20.51175)` y `core[3].relativeLocation` idéntico — prueba directa, sin captura ni opinión.

365. 🔴🔴 **Al replicar una fórmula de HLSL en Blueprint, los `saturate` y los `clamp` también son parte de la fórmula.** `GetBlobFitScale` copiaba `max(fac, MinScl)` del shader pero el shader dice `max(fac, **saturate**(MinScl))`. Mientras `MinScale` se autoró dentro de 0-1 los dos coincidían; el día que Beltrán la subió a **1,51** para agrandar las gotas chicas, el shader la leyó como 1,0 y el BP como 1,51 → la esfera anclada quedó ~50% más grande que su gota. 💡 **La divergencia no aparece el día que escribís la copia, sino el día que alguien saca la perilla del rango donde las dos versiones coincidían.** Si hay que duplicar una fórmula, copiar el cuerpo ENTERO — saturates, clamps, mins — y anotar en el tracker que está duplicada.

366. 🔴 **Escribirle el DESTINO a un objeto por frame desde afuera pelea con su propia interacción.** El chain le empujaba `TargetLoc` = centro de la gota a la esfera anclada **en cada tick, sin condición**, mientras el beam le empujaba la posición de la mano a la misma variable: al agarrarla los dos escribían el mismo destino y la esfera tironeaba de vuelta a la gota (*"me cuesta agarrar las esferas cuando ya están ancladas"*). ✅ El push externo va **guardado por el estado de la interacción** (`if not (Moving or Grabbed)`), igual que ya estaba la posición. 💡 La sospecha del usuario apuntaba a otra cosa (los adornos nuevos, "interfieren con el collision"): un `trace_world` por el centro devolvió `null` y los descartó en una llamada. **Medir primero el sospechoso obvio ahorra reconstruirlo.**

367. 🔴🔴 **Un material de raymarch reusado sobre una malla ESCALADA: `MaxT` esta en unidades LOCALES, y el alcance real en mundo es `MaxT x escala`.** `M_SlotChain_SC` arranca el rayo en `CameraPositionWS` transformada a local, asi que `MaxT` mide **desde la camara**, no desde la superficie. En el proxy del metaball (escala 1) local = mundo y no se nota; al reusar el shader en las esferas (escala ~0,4) un `MaxT` de 140 dio **57 cm de alcance** — sintoma exacto: *"solo aparecen cuando me acerco con la camara"*. ✅ `MaxT` enorme (40.000) y listo: **no cuesta nada**, porque el sphere tracing converge en pocos pasos y el trabajo lo acota `Steps`, no `MaxT`. 💡 Diagnostico: si algo aparece/desaparece **por distancia de camara** y ya descartaste LODs y draw distance, en un raymarch mira SIEMPRE el origen del rayo — `get_expression_inputs` sobre el nodo que alimenta `RayOrigin` lo dice en una llamada. Mi primera hipotesis (cobertura de la SDF dentro del proxy) era falsa, y el usuario la refuto en un paso subiendo el radio "a infinito" sin que cambiara nada.

368. 🔴 **Un translucido DENTRO de otro translucido se lava aunque este geometricamente delante: entre dos translucidos NO hay test de profundidad.** Las esferas ancladas viven al 90% dentro de su gota del metaball, y se veian apagadas (*"cuando las esferas estan dentro se ven muy poco"*). No era opacidad: los dos son `BLEND_Translucent`, asi que el orden lo decide la **distancia al origen de los bounds** (§276) y el proxy de la cadena es una malla enorme cuyo centro esta en el medio del arco — gana a unas esferas y pierde con otras, **inconsistente por slot**. ✅ El lever es `TranslucencySortPriority`, no la opacidad: bajarle la opacidad a la cadena cuesta el look autorado y **no arregla el orden**. Pila de la estacion: **cadena 0 · nucleos 10 · esferas 20**, empujada desde `ApplyLook` con la perilla `OrbSort`.
    💡 **Donde poner la perilla cuando el actor se SPAWNEA.** `BP_SoundOrb_SC` no existe en el editor, asi que una variable suya no se puede autorar mirando. La perilla vive en el **director colocado** y viaja al componente por la funcion que ya empuja el look (`ApplyLook`) — con eso la toman **las esferas de juego y las previas del editor** con un solo nodo, y §325 (el CDO no llega a los actores ya colocados) deja de aplicar porque nadie serializa el valor.

369. ⚠ **`remove_function_graph` no libera el NOMBRE hasta que compilas.** Si haces `remove_function_graph("UpdateSpin")` y acto seguido `add_function_graph("UpdateSpin")`, el grafo nuevo nace como **`UpdateSpin_0`** — y entonces `write_graph_dsl` con la cabecera `(fn UpdateSpin ...)` falla con **`AddEvent|UpdateSpin does not exist`**, porque el DSL exige que el nombre de la `fn` coincida con el del grafo y, al no coincidir, intenta crear un evento. ✅ La receta completa es `remove_function_graph` → **`compile_blueprint`** → `add_function_graph` → params → `write_graph_dsl` → compile; el compile intermedio es lo que purga el objeto viejo. Si ya quedo el `_0`, se sale del pozo borrando ESE (`remove_function_graph("UpdateSpin_0")`), compilando, y volviendo a crear.
    💡 Y el modulo de float **no se llama `Fmod`**: es **`Math|Float|%(Float)`**. Ante un nombre de nodo inventado, `find_node_types` con la categoria y una barra al final (`Math|Float|`) lista todo lo que existe de verdad en una sola llamada.

370. 🔴🔴 **`SceneTools.find_actors` busca por la ETIQUETA del actor, no por su nombre de clase — un `[]` NO prueba que no exista.** 2026-09-24: `find_actors(name="Sensor_Soul")` devolvio **vacio** y de ahi conclui "el sensor se spawnea, no hay instancia colocada, con el CDO alcanza". Falso: `find_actors(name="Sensor")` devolvia `BP_Sensor_Soul_C_0`, colocado en `TestMeshes`. Sobre esa conclusion falsa se entrego un puntero con **las siete perillas en 0** en la instancia — escala 0 y color negro, o sea **invisible**, que es [[instance-editable-nace-en-cero]] por decimoquinta vez. Lo mas caro: en el mismo commit yo habia **escrito la advertencia** (*"si aparece un puntero invisible en otro nivel, mira `PtrLen` en la instancia"*) y aun asi no la aplique, porque le crei a la busqueda.
    ✅ **Para saber si una clase tiene instancias colocadas, buscar por el PREFIJO corto** (`"Sensor"`, no `"Sensor_Soul"`) o por tag, y **en la duda, escribir igual la instancia**: escribir defaults a un actor que no existe no cuesta nada, dar por inexistente uno que si existe cuesta una pasada del usuario con las gafas puestas.
    🚩 **Y la forma general, que es la que se repite:** una perilla nueva que multiplica una escala o alimenta un color **falla en silencio y de forma total** cuando nace en 0 — no se degrada, desaparece. Cada vez que se agrega una variable `instance_editable` que escala, colorea o dimensiona algo, **el paso siguiente no es compilar, es escribir la instancia y leerla de vuelta**.

371. 🔴🔴 **Un nodo IMPURO usado como expresion no se ejecuta nunca y devuelve 0, en silencio y compilando en verde.** 2026-09-24, `LookOffset` del director: las tres llamadas a `Hash01` se escribieron **dentro del `return`**, sin `bind`. El DSL creo los nodos y conecto sus pines de datos — el `read` las muestra perfectas, con `Index` y `Salt` cableados — pero **`Hash01` es impura** (tiene pines de exec) y quedo fuera de la cadena: `FunctionEntry.then` iba directo al `FunctionResult`. Un nodo impuro que no se ejecuta entrega el **default de su salida**, o sea 0.
    🧪 **La firma en los datos es inconfundible y hay que saber leerla:** las 8 esferas se desplazaron **exactamente `-Spread`** en los tres ejes. Offset = `(hash*2-1)*Spread`, asi que `-Spread` exacto significa `hash == 0` **exacto**, tres veces, en ocho indices distintos. Un hash real no hace eso jamas. **Cuando un valor aleatorio sale constante y ademas cae justo en un extremo del rango, no es mala suerte: es el default de algo que no corrio.**
    ✅ Dos formas de arreglarlo: **`bind`ear cada llamada** (que es lo que hacen `LookLobe`/`LookColor`, y por eso ellas si funcionan), o meter los nodos a mano en la cadena de exec. Aca se hizo lo segundo por cirugia — `entry → hash1 → hash2 → hash3 → result` — para no borrar la funcion y romper la llamada que le hace **otro Blueprint** (`BP_SoundOrb_SC`), que no se re-resuelve sola como las del mismo BP.
    🚩 **Regla:** antes de inlinear una llamada a funcion propia dentro de una expresion, mirar si tiene pines de exec. Si los tiene, **va en un `bind`**. Y `read_graph_dsl` **no** delata esto: muestra los datos conectados y no dice nada del exec.

372. 🔴🔴 **REORDENAR una cadena de exec crea un bucle infinito si no rompes el enlace viejo A MANO.** Es el §338 otra vez, y volvio a morder el 2026-09-24. `UpdateAttract` estaba PRIMERO en el Tick y se lo quiso mover al FINAL. Se hizo `EventTick.then → UpdateSpin` (que **si** desplazo el enlace viejo, porque un pin de exec de SALIDA es de enlace unico) y `UpdateVisual.then → UpdateAttract`. **Pero `UpdateAttract.then → UpdateSpin`, del orden anterior, seguia ahi** — nadie lo desplazo, porque el que se reescribio fue el pin de salida de OTRO nodo. Resultado: `… UpdateVisual → UpdateAttract → UpdateSpin → …` en ciclo, y Unreal aborto el PIE con *"Infinite loop detected"* apenas se le dio play.
    ✅ **Mover un nodo dentro de una cadena son TRES operaciones, no dos:** conectar el nuevo predecesor, conectar el nuevo sucesor, y **`break_pins` del enlace que el nodo movido tenia hacia su sucesor viejo**. La asimetria que lo hace invisible: reconectar una SALIDA desplaza sola lo que hubiera; el enlace que sobra siempre es el que **sale del nodo que moviste**.
    🧪 **Y se detecta gratis:** el `read_graph_dsl` del EventGraph despues del cambio muestra la cadena linealizada. Si un nodo aparece dos veces, o la lista no termina donde esperabas, hay ciclo. Compilar **no** lo detecta — compilo en verde y exploto en play.

373. 🔴🔴 **`remove_function_graph` ROMPE a los que llaman esa función desde OTRO Blueprint, y el compile del Blueprint dueño sale en verde.** 2026-09-24, dos veces en la misma jornada: `BP_Stroke:ApplyLook` se reconstruyó con la receta limpia (`remove` → `compile` → `add` → `write`), pero el nodo que la llama vive en **`BPC_DrawTool_NC:Press`**. Entre el `remove` y el `add`, Unreal le rompió los pines a ese nodo, y el mensaje quedó **solo en el log del OTRO asset**:
    ```
    [Compiler] Could not find a function named "ApplyLook" in 'BP_Stroke_C'.
    [Compiler] In use pin  Target  no longer exists on node  Apply Look .
    ```
    `BP_Stroke` compilaba perfecto todo el tiempo. El síntoma le llegó a Beltrán como un **`Blueprint failed to compile: BPC_DrawTool_NC`** en el editor, sin que ninguna de mis llamadas hubiera devuelto error.
    ✅ **La receta de reconstrucción tiene un cuarto paso cuando la función es pública: recompilar Y GUARDAR a cada llamador.** El `add_function_graph` la re-resuelve por nombre, así que un `compile_blueprint` del llamador alcanza para sanarlo — pero si no lo hacés, queda roto en memoria; y si lo hacés sin guardar, queda sano en memoria y **roto en disco**. La segunda vez fue justo eso: recompilé y me olvidé de incluirlo en el `save_assets`.
    🧪 **Cómo detectarlo sin esperar a que el usuario lo vea:** `EditorToolset.LogsToolset.GetLogEntries(pattern="<NombreDeLaFunción>")`. Filtra por nombre de función y muestra los errores de compilación **de todos los assets**, que es exactamente lo que el compile del asset que estás tocando no te cuenta. 📌 Vale como regla general: **después de tocar cualquier cosa PÚBLICA (función, variable, parámetro de material), el chequeo barato es el log filtrado por ese nombre**, no el compile del asset dueño.

373. 🚩 **Una perilla cuyo rango util empieza en un numero que el usuario no conoce es una trampa — y el sintoma es que el efecto DESAPARECE entero.** 2026-09-24: `AttractMax` de `NS_OrbAttract_SC` es la distancia esfera→mano por debajo de la cual se emiten particulas. Pero la esfera agarrada se queda fija a **`GrabHoldDist` = 80 cm**, asi que **cualquier valor menor a 80 apaga el efecto por completo**. Beltran arrastro el slider a **56,3** afinando, y reporto *"ya no veo las particulas"*. No habia nada roto: la condicion `80 < 56,3` simplemente nunca se cumple.
    🔍 **Como se encontro sin adivinar:** leer **las perillas de la instancia**, no el codigo. Los seis valores salieron con decimales largos (0,887 · 122,84 · 0,616 · 41,8 · 56,3) — firma inconfundible de sliders arrastrados a mano. **Ese patron por si solo dice "lo movio el usuario, compara contra el rango valido"**, y ahi salta cual quedo fuera.
    ✅ Mitigacion de diseno, para la proxima: **expresar el umbral como MARGEN sobre la magnitud de la que depende** (`emite si dist < GrabHoldDist + Margen`) en vez de como distancia absoluta. Con margen, el 0 sigue funcionando y **no existe un valor que mate el efecto**. Una perilla absoluta acoplada a otra constante del sistema necesita, como minimo, que el nombre o la doc digan de que constante depende.
    🏁 **Epilogo (mismo dia):** no se mitigo — **se borro la perilla**. Al rehacer el movimiento a velocidad constante (ver §374) la compuerta de distancia dejo de hacer falta y `AttractMax` se elimino de los dos Blueprints. Cuando una perilla es una trampa **y ademas no hace falta**, sacarla le gana a documentarla.
374. 🚩 **Si una magnitud tiene que ser CONSTANTE, escribela directo; no le pidas a un sistema de fuerzas que la mantenga.** 2026-09-24, el cono de `NS_OrbAttract_SC`. Beltran queria *"velocidad constante suave"* y tenia tres fuentes de variacion encima, todas invisibles desde el panel: (a) `PointAttractionForce` con **`Use Falloff` activo** → la aceleracion depende de la distancia al blanco; (b) el blanco esta a distancia variable, asi que hay mas o menos **pista para acelerar**; (c) `InitializeParticle` con **masa aleatoria 0,75-1,25** → `SolveForcesAndVelocity` divide la fuerza por masa y reparte aceleraciones distintas. Sumadas, la "velocidad" era una funcion de cuatro cosas y de ninguna perilla.
    ✅ **La receta de velocidad constante hacia un punto, con modulos de fabrica:** `AddVelocity` en **Particle Spawn**, modo **"From Point"**, `Velocity Origin` = el punto destino y `Velocity Speed` **NEGATIVA** (el modo calcula `normalize(pos − punto) · speed`, asi que el signo invierte el sentido). Rapidez exacta, fijada al nacer, sin inercia ni masa de por medio. Despues: `Drag` **apagado y en 0** (todo drag hace decaer la velocidad) y `PointAttractionForce` con `Attraction Strength` **0**, conservado solo como **matador** por `Kill Within Radius`.
    🔑 **El bonus:** con velocidad constante, **vida = recorrido / velocidad** es exacta. Linkeando `Lifetime Min` y `Lifetime Max` al mismo user param, el cono **termina siempre en el mismo sitio** sin depender de que el matador acierte. La punta no queda dura porque las particulas nacen repartidas en la esfera de `Shape Location` y mueren repartidas igual.
    🚩 **Y la leccion de lectura de reportes:** *"la perilla X hace que vayan mas rapido"* puede ser **cierto por un camino que no esta en el nombre de X**. `AttractMax` era una distancia de compuerta y terminaba fijando la velocidad de llegada. Antes de contestar "esa perilla no toca la velocidad", buscar el camino indirecto — el usuario esta describiendo lo que ve, no lo que cree del codigo.


374. 🔴🔴 **Un actor spawneado DURANTE la inicialización del mundo recibe su `BeginPlay` DIFERIDO al final del frame — así que pisa todo lo que hayas escrito después de spawnearlo.** 2026-09-24: `BPC_DrawTool_NC.Setup` (que corre en `Event Possessed`) hacía `PullDirector` → `EnsurePalette` (spawnea `BP_PincelSelect`) → `ApplyStartBrush`. La paleta, en **su** `BeginPlay`, inicializa su selección y llama `CambiarPincel(0)`. Como ese `BeginPlay` **no corre dentro del spawn** sino al final del frame, terminaba corriendo **después** de `ApplyStartBrush` y dejaba el pincel en 0. Síntoma para el usuario: *"no dibujo el tubo, solo el trazo"*, dos veces seguidas.
    🧪 **Lo decisivo fue MEDIR, no razonar**: el `PrintString` dentro de `ApplyStartBrush` imprimía **3**, y sin embargo `get_properties` sobre la instancia del **mundo de PIE** (`/Game/.../UEDPIE_0_<Nivel>...`) devolvía **`BrushIn: 0`**. Escritura correcta + valor final incorrecto = **hay un segundo escritor, y corre después**. Es la firma de [[verificar-estado-estable-no-spawn]], y las dos hipótesis previas (que el nodo no se hubiera cableado, que el director no se encontrara) eran falsas.
    ✅ **Leer la instancia del mundo PIE es la medición barata que zanja esto**: `StartPIE` → `get_properties` sobre `UEDPIE_0_...` → `StopPIE`. Tres llamadas, y distingue "no se escribió" de "se escribió y lo pisaron", que por síntoma son idénticos.
    ✅ **El arreglo**: diferir la escritura con `Utilities|Time|SetTimerbyFunctionName(self, "ApplyStartBrush", 0.5)` al final de `Setup`. 📌 Regla general: **lo que quieras que gane sobre el `BeginPlay` de un actor que vos mismo spawneás, no lo escribas en la misma función que lo spawnea.**
375. 🧹 **Un `Set*ParameterValueOnMaterials` con un nombre que el material no tiene NO falla, no avisa, y sostiene perillas fantasma aguas arriba.** 2026-09-24, `BP_OrbDirector_SC::ApplyLook`: era la lista de empujes del material viejo (`M_SoundOrb_SC`, ameba) con 4 nombres de la cadena pegados al final, pero las esferas ya usaban `MI_OrbBlob_SC`. De 20 empujes **llegaban 5**; 14 apuntaban a parámetros inexistentes y 1 (`FloatAmount`) existía como **Scalar** y se empujaba con el setter **Vector**. Consecuencia real: 5 perillas del panel del director que el autor arrastraba (`Brightness` 1,143 · `ShadeSharp` 1,4747 · `FloatScale` 0,12005 — decimales largos = las movió a mano) **sin que pasara nada**.
    🔍 **La verificación es UNA llamada:** `MaterialInstanceTools.list_parameters(<el MATERIAL padre, no la instancia>)` devuelve los parámetros que expone compilado; se cruza contra los nombres que empuja el Blueprint y se acabó la discusión. Hacerla **siempre que un BP cambie de material**, que es justo cuando se hereda una lista de empujes vieja.
    🚩 **El patrón general, que es el que hay que reconocer:** cuando un Blueprint se repunta a un material nuevo, los `Set*Parameter` viejos **sobreviven al cambio** porque nada los rompe. Y como cada uno suele estar alimentado por una variable, cada empuje muerto **le da coartada a una perilla muerta**. Limpiar el empuje es lo que permite borrar la perilla; al revés no se puede, porque el getter todavía la referencia.
    ⚠ Al borrar duplicados tipo `WobbleFreq_0`, **releer `list_variables` y la función que usa el original** después: comparten FName base y un borrado por nombre es el sitio ideal para llevarse el que sí vivía.
376. 📏🔴 **Una captura de performance con el visor EN LA MESA no mide nada — y el número que da es creíble, que es lo peligroso.** 2026-09-24, primera medición del APK de `TestMeshes`: el CsvProfiler devolvió `FrameTime` mediana **24,67 ms** y `GameThreadTime` **24,65 ms**. Leído así, el veredicto obvio sería "estás a 40 fps y sos CPU-bound". **Es falso.** Al soltar el sensor de proximidad, la Quest baja los clocks de CPU/GPU y capa la app; el `GameThread` se infla solo por eso.
    🔎 **La columna que lo delata es `MaxFrameTime`:** salió **33,33 ms constante** = cap de 30 fps = nadie tenía el casco puesto. Con el visor puesto vale **13,89 ms** (72 Hz). Es un canario gratis que viene en todos los CSV del CsvProfiler, así que **filtrar por él antes de mirar cualquier otra métrica** — `scripts/read_csv_perf.py` lo hace y directamente se niega a resumir una captura inválida.
    ✅ Para medir solo, sin ayudante: `scripts/quest_perf.ps1 measure -Seconds 60` da 20 s para ponerse el visor, graba, corta y trae el CSV.
    🚩 **La forma general:** un instrumento que devuelve un número plausible cuando el sujeto no está en el estado que se quiere medir es peor que uno que falla. Antes de creerle a una medición de device, buscar **la columna que prueba que el estado era el correcto** — acá el cap de frame rate; en Breath fue el control positivo del sensor. Hermana de [[debugging-instrumento-sin-validar]] y [[verificar-estado-estable-no-spawn]].
    ⚙ Y para que el CSV traiga tiempo de GPU (que en Quest es EL número, por fill-rate) hay que encender `r.GPUStatsEnabled 1` + `r.GPUCsvStatsEnabled 1`; sin eso la columna `GPUTime` viene vacía.
377. 📸 **Un thumbnail/preview capturado justo después de `recompile` puede salir NEGRO sin que nada esté roto.** 2026-09-24: tras la cirugía del march de `M_SlotChain_SC`, `CaptureAssetImage(MI_OrbBlob_SC)` devolvió solo el fondo a cuadros, mientras el viewport mostraba la cadena perfecta con el MISMO master. Era la **compilación asíncrona de shaders**: el thumbnail se re-renderiza con el shader todavía en vuelo. Re-capturar unos segundos después lo trajo de vuelta idéntico. Antes de diagnosticar "el cambio rompió el material" por un preview negro: (a) mirar si otra vista con el mismo master renderiza bien, (b) repetir la captura, (c) el árbitro es el log (`Failed to compile` / `error X…`), no la imagen.
378. 🔴🔴 **Comparar el CUADRO ENTERO no verifica nada: el elemento que estás optimizando puede haber desaparecido del todo y la foto se ve "igual".** 2026-09-25, el peor error de verificación de esta etapa. Tras la cirugía del march de `M_SlotChain_SC` capturé el viewport con cámaras idénticas antes/después, miré las dos imágenes de 1107 px, las declaré iguales y escribí en el tracker *"idénticas salvo la pose animada"*. **El metaball de la cadena había desaparecido por completo.** Lo vio Beltrán en su viewport a la mañana siguiente (*"ya no se ve el metaball del sequencer"*), no yo.
    🔍 **Por qué se me escapó, que es la parte reutilizable:** la cadena es un lavado **pálido, translúcido y de bajísimo contraste** (crema sobre un fondo verde claro) que ocupa una banda angosta del cuadro. A tamaño completo, su ausencia no cambia la "impresión general" de la imagen; los slots, los iconos y las 20 esferas de colores siguen ahí y dominan la lectura. **Un elemento de bajo contraste se evalúa comparando el elemento, no la escena.**
    ✅ **El método que SÍ funcionó, y es el que hay que usar siempre:** recortar el **bounding box del elemento** en las dos capturas, **ampliarlo ×2** y apilarlos con etiqueta. Con eso la diferencia fue obvia al primer vistazo. Tres estados (antes · con el cambio · restaurado) prueban además que el cambio es la causa, no una coincidencia.
    🚫 **El diff numérico de píxeles NO sirve como sustituto acá**: el material anima por `Time`, así que dos capturas nunca son idénticas — el "antes vs restaurado" (mismo material) dio 4,7 % de píxeles distintos, el mismo orden que "antes vs roto". Un umbral de diff sobre una escena animada mide la animación, no el cambio.
    🚩 **Y el PIE verde tampoco lo agarró:** la corrida imprimió `esferas=20`, `pad ON` y cero `Accessed None` — porque el log mide **lógica**, no píxeles. Es otra cara de la regla del proyecto: *lo que se MIDE se verifica por log; lo que se VE, se verifica mirando el elemento*. Un `TEST PASS` nunca dice que algo se dibuje.
    📌 Corolario para materiales: **antes de tocar un shader, guardá un recorte de referencia del elemento** (mismo encuadre, ampliado). Es el control positivo, cuesta una captura, y sin él no hay forma honesta de decir "se ve igual".
379. 📉🔴 **El ruido entre sesiones de una captura de juego libre en la Quest es ~15 %, no ~4 % — y eso vuelve INCOMPROBABLE cualquier optimizacion mediana.** 2026-09-25, midiendo la estacion Attracting: el **mismo build sin optimizar** dio **21,65 ms** en una pasada y **24,85 ms** en otra. Con esa dispersion, la medicion posterior del build optimizado (24,30 ms) no dice absolutamente nada: cae dentro del rango que ya daba la version sin tocar.
    🔍 **La causa es el protocolo, no el dispositivo.** La captura es de *juego libre* ("agarra esferas, movete"), y en una escena fill-rate bound lo que manda el coste es **cuanta pantalla cubren los objetos caros** — o sea hacia donde mirabas y cuantas esferas tenias cerca. Eso cambia entre pasadas mucho mas que cualquier optimizacion razonable. El estado termico entre sesiones suma otro tanto.
    ✅ **La forma correcta: A/B DENTRO DE UNA SOLA SESION, alternando configuraciones cada ~20 s.** Comparten escena, pose y temperatura, que es justo lo que contamina la comparacion entre sesiones. Para que un cambio de **shader** sea alternable en runtime: parametro **escalar** `FastPath` con `if (FastPath > 0.5) {nuevo} else {viejo}` en el HLSL (un **static switch NO sirve**: es de tiempo de compilacion), un Blueprint que lea una cvar propia con `GetConsoleVariableFloatValue` y la empuje a los MIDs, y `quest_phases.ps1` alternando la cvar por broadcast de adb. Es el mismo principio del control positivo: **la comparacion tiene que variar UNA sola cosa.**
    🚩 **El error de razonamiento a evitar:** aceptar como "linea de base" **una sola captura**. Un unico numero no tiene barras de error, asi que invita a leer cualquier diferencia posterior como senal. La linea de base de una metrica ruidosa son **al menos dos pasadas**, y su separacion **es** la resolucion del instrumento — si el cambio que buscas es menor que esa separacion, no lo vas a ver por mas que midas.
    ⚠ Y al comparar dos CSV con `read_csv_perf.py`: su **"VEREDICTO: CPU BOUND"** asume que el segundo archivo es la fase B de baja resolucion del test de Meta. Si le pasas dos capturas de resolucion normal, el veredicto es **basura** — la herramienta no puede saber que le diste otra cosa.

375. 🔴🔴🔴 **Una función nueva que comparte MATERIAL con una vieja la rompe por el DEFAULT del parámetro, no por el push.** 2026-09-24/25: al construir el pincel de tubo se agregaron `BeadAmp`, `ShadeAmount` y `FacetAmp` a `M_Emissive` — el material que **también usa la cinta ya aprobada**. Se tuvo el cuidado de empujar esos parámetros **solo desde `Tube_StartStroke`** para no tocar la cinta… y aun así la cinta sali con el ancho ondulado, porque **`BeadAmp` se cre con `defaultValue = 0.6` en el material padre**. El push no era el canal: **el default lo es**. Beltrán: *"lo mezclaste con el del tubo… un nuevo pincel no puede romper otro que ya estaba aprobado"*.
    🔴 **Y lo caro es que YO HABÍA ESCRITO LO CONTRARIO en el tracker** (*"sus defaults en el material padre son 0, o sea que la cinta queda exactamente igual"*) **sin leerlos**. Es [[soul-charger-declarado-no-aplicado]] otra vez: afirmar un valor efectivo en vez de verificarlo. Al leerlos: `ShadeAmount` 0 ✅, `FacetAmp` 0 ✅, **`BeadAmp` 0.6 ❌**.
    ✅ **La regla: al agregar un parámetro a un material COMPARTIDO, su default tiene que ser el NEUTRO (el valor que deja el aspecto anterior intacto), y el valor artístico se empuja desde quien lo quiere.** El default es el contrato con todo lo que ya existía.
    🧪 **Verificación barata y obligatoria**: después de agregar parámetros a un material compartido, leer el `defaultValue` de **cada uno** (`get_properties` sobre la expresión) y confirmar que es neutro. Tres llamadas. Aquí habría ahorrado que un pincel nuevo degradara uno ya aprobado en visor.
    📌 Corolario de proceso, dicho por el usuario: **una función nueva no puede degradar una ya aprobada**. Si van a compartir un asset, el asset necesita un modo neutro por default; si no se puede, el nuevo se lleva su propia copia (como se hizo bien con `M_Tube_Inst` para `TwoSided` y `Opaque`).
380. 🧪 **Receta: conmutar un cambio de SHADER en runtime dentro del APK, para poder medirlo A/B en una sola sesion.** Montada el 2026-09-25 para decidir si la tecnica de blobs raymarcheados es viable en Quest. Las cuatro piezas, y por que cada una:
    1. **Material Parameter Collection** (`MaterialTools.create_parameter_collection`, y los escalares se agregan con `ObjectTools.set_properties` pasando `scalarParameters: [{parameterName, defaultValue, id}]` — el `id` es un GUID que hay que inventar, y **`parameterId` NO es escribible en el nodo que la lee**, se deriva solo). Se elige MPC y no un parametro por material porque **una sola escritura llega a todos los materiales** que la leen, sin tocar ningun Blueprint de la mecanica.
    2. En el material, un **`MaterialExpressionCollectionParameter`** (`collection` + `parameterName`) enchufado como entrada nueva del nodo Custom. Agregar una entrada a un Custom exige **reenviar el array `inputs` COMPLETO** con la nueva al final; la forma de una entrada escalar es `{"inputName":"X","input":{"expression":{"refPath":...},"outputIndex":0,"inputName":"None","mask":0,...}}`.
    3. 🔴 **Parametro ESCALAR, nunca un static switch**: el static switch es de tiempo de compilacion y no se puede alternar en runtime. Como el escalar es uniforme, la rama tambien lo es y **no cuesta nada en el modo por defecto**.
    4. **`ke * <Evento>`** por broadcast de adb dispara un evento custom de Blueprint que escribe la coleccion. Verificado en el motor, no por folclore: `FParse::Command(&Cmd, TEXT("KE"))` esta en `UnrealEngine.cpp:5838`, dentro de **`UEngine::Exec_Dev`**, que arranca bajo `#if !UE_BUILD_SHIPPING` → **funciona en Development y NO en Shipping**.
    ⚠ **El nodo de MPC colisiona por nombre**: `find_node_types` devuelve `Rendering|Material|SetScalarParameterValue` dos veces y el DSL agarra el de **MaterialInstanceDynamic** (pin `self` de tipo MID). El que va es el de la coleccion, y se crea por cirugia con **`declaring_class = /Script/Engine.KismetMaterialLibrary`**; se reconoce porque la clase del nodo resultante es `K2Node_CallMaterialParameterCollectionFunction` y sus pines son `Collection` / `ParameterName` / `ParameterValue`. Misma familia que la trampa de los Fades de AudioComponent.
    ✅ **Y el control positivo es obligatorio**: antes de medir, disparar un modo y **comprobar en logcat que el Blueprint contesto**. Si `ke` no llega, las fases no cambian nada y la medicion sale plausible y falsa — exactamente el modo de falla de [[debugging-instrumento-sin-validar]]. `quest_perfmodes.ps1` lo hace y aborta si no ve el eco.

381. 🔀 **El split del banco PerfMode (modos 4/5) y dos datos operativos que dejó montarlo** (2026-09-25).
    1. **Partir el costo de un material compartido entre sus DOS consumidores sin tocar ningún BP:** cuando un master lo usan dos superficies distintas (acá la cadena, MID sin overrides, y las esferas, `MI_OrbBlob_SC`), un parámetro que ya difiere entre ellas sirve de discriminante dentro del shader — acá `WobbleAFS.x` (0 en la cadena, 0,09 en las esferas). `modo 4 = return si WobA>0` apaga solo las esferas; `modo 5` al revés. Y la aditividad `piso = m4 + m5 − m0` **deriva el piso real que el cap de 72 Hz esconde en el modo 3**. Verificación del discriminante: **preview de asset de cada camino** (`CaptureAssetImage` del MI y del master) — el viewport del nivel no sirve si las dos superficies están co-ubicadas (en la galería los orbes se anclan en el centro de las gotas).
    2. ⚠ **Los errores de VALIDACIÓN de argumentos de `execute_tool` ESCAPAN del `try/except BaseException` del sandbox** (visto 2×: `material_or_function` y `node_class` con path corto). Dato nuevo: en ambos casos **NO dispararon Undo** (log `LogEditorTransaction` vacío) y el `set_properties` previo del mismo script sobrevivió — la validación parece pre-transacción. **NO relaja §60** (2 muestras contra un incidente real con actores perdidos): plantilla y canario siguen obligatorios; pero ante un escape así, **verificar el estado real antes de asumir que se perdió el trabajo** (esta vez el trabajo estaba intacto).
    3. `AssetTools.write_file` es **`file_path`**, no `path`; `CaptureAssetImage` devuelve `{mimeType, data}` plano (no `{image:{data}}`); `CaptureViewport` exige `captureTransform` explícito; `save_assets` es **`asset_paths: [string]`**.

382. ⏱️ **El "trabado" progresivo de animaciones de MATERIAL en el APK es el reloj en fp16 — y se puede reproducir AL INSTANTE en vez de esperar 15 minutos de visor.** Síntoma (Attracting, 2026-09-25): a más tiempo de sesión, el wobble/metaball más "pegado", mientras manos y mundo corren fluidos; en editor jamás se ve (PC = fp32). Causa: los inputs de un Custom node llegan como `MaterialFloat` (= half en móvil); `Time` acumula y a T≈1000 s su ulp en half es ~1 s. Dos curas según el material: **(a) Custom node → leer `float Tt = View.GameTime;` adentro del HLSL** (uniform fp32, idéntico al nodo Time pelado; costo cero, sin period, sin saltos); **(b) material de grafo → `MFPM_Full_MaterialExpressionOnly` + Time con Period** (la cura de los degradados, memoria del 2026-08-18). ⚠ No flipear el master del raymarch a full precision: en Adreno el fp16 corre a 2× — duplicaría el ALU del march. ⛔ **El control que parecía obvio NO FUNCIONA, y el fallo enseña más que el truco:** se montó un modo de banco que suma +1024 s al reloj por el camino half (`half hT = half(Tt + 1024.0)`) contra otro que suma lo mismo en fp32, esperando trabado instantáneo vs fluido. **En device los dos se vieron iguales.** Y eso NO es "no se nota": a T≈1024 el ulp de un half es **exactamente 1,0 s**, o sea que debía verse congelado a saltos de un segundo. La lectura correcta es que **un `half` explícito en un Custom node NO garantiza fp16 real** — el cross-compiler de UE lo promueve a float según el target. 🔴 Consecuencia doble: (a) ese control es inválido como verificación, y (b) **si el half explícito no es fp16, el diagnóstico fp16 del síntoma también queda sin probar**. Antes de creerle a un control de precisión, hay que **demostrar que el camino "malo" efectivamente degrada** (si el control negativo no rompe nada, no es un control). El test que sí decide acá es el aburrido: una sesión larga de verdad. Ver modos 6/7 de `M_SlotChain_SC` + `BP_PerfSwitch_SC` y [[debugging-instrumento-sin-validar]]. 🔴 Deuda: TODOS los materiales animados por Time de la obra (15 min) comparten el bug latente.

383. 🔴🔴🔴 **Un `execute_tool_script` que falla puede CRASHEAR el editor, no solo perder trabajo. El asesino es el UNDO, no el script.** Medido el 2026-09-25 construyendo `BP_TBStroke`: **dos crashes, el mismo mecanismo exacto**. El log lo muestra igual las dos veces — el error del script, despues `LogEditorTransaction: Undo Execute tool script`, y ahi el `EXCEPTION_ACCESS_VIOLATION`:

    | # | Que habia mutado el script antes de fallar | Donde crasheo |
    |---|---|---|
    | 1 | expresiones de un Material + `set_properties` sobre ellas | `UnrealEditor_MaterialEditor` |
    | 2 | `AssetTools.duplicate` de un NIVEL, ya creado y a medio cargar | `UnrealEditor_RenderCore` |

    El patron: el rollback deshace **estado pesado del editor que otro subsistema todavia referencia** (el grafo del Material Editor; un world que el renderer tiene enganchado) y lo deshace sin avisarle a nadie. Con variables, funciones y `write_graph_dsl` el Undo es barato y no pasa nada — las 20 tandas de esta sesion que solo tocaban eso fallaron varias veces sin consecuencia.

    ✅ **Las reglas que salen de esto:**
    - **Materiales y NIVELES nunca dentro de `execute_tool_script`.** Un `create_material`, `add_expression`, `connect_expressions`, `duplicate` de `.umap`, `load_level` o `add_to_scene` va en su **propia llamada directa** `call_tool`. Un fallo ahi es un fallo atomico, sin Undo multi-paso.
    - En un script, **todo lo de lectura y validacion primero**, y las mutaciones al final y baratas.
    - 🔴 **GUARDAR despues de CADA tanda que ande** (`AssetTools.save_assets` con rutas explicitas). En esta sesion el primer crash se llevo 38 variables + un componente porque no habia guardado; el segundo no costo **nada** porque cada paso estaba guardado. Es la diferencia entre media hora perdida y cero.
    - Y no reconstruir encima de un asset que acaba de sufrir un Undo: **borrarlo y recrearlo**. El primer crash paso justo por eso (`get_expression_output_names` sobre un material recien revertido).

384. ⚠️ **`SceneTools.load_level` falla si el nivel ACTUAL tiene cambios sin guardar** (*"Cannot load 'X': the level has unsaved changes"*), y en un script ese fallo se convierte en el crash de §383. El nivel actual sucio no es necesariamente nuestro — puede venir del arranque del editor o de un crash previo. ✅ Antes de un `load_level`, comprobar con `AssetTools.is_dirty` del nivel actual, y si esta sucio **preguntarle al usuario** en vez de guardarselo (guardar el nivel de otro es tocar su trabajo). O mas simple: pedirle que abra el nivel a mano y seguir desde ahi.

385. 🔴🔴 **Al trocear geometría que crece, el trozo es una VENTANA DE DIBUJO, no una frontera lógica.** Construyendo `BP_TBStroke` el mismo malentendido produjo **dos bugs visibles y dos latentes**, en dos viajes al visor: (1) el trazo se ponía **negro** pasado cierto largo — cada trozo es una `MeshSection` nueva y solo la 0 tenía material; (2) aparecía un **corte de ~3,6 mm** en cada costura — el kernel de suavizado `0.3/0.4/0.3` se evaluaba de forma **asimétrica** a cada lado (el trozo viejo clampeaba `next` a sí mismo, el nuevo clampeaba `prev` al inicio del trozo); y latentes, la **UV** y el **suavizado de presión** se reiniciaban en cada trozo.

    ✅ **La regla:** todo lo que se inicializa "al empezar un trazo" hay que revisarlo en el corte de trozo — **un trozo nuevo NO es un trazo nuevo**. En concreto: los índices de vecino se clampean al **array completo** (`max(i-1, 0)`, `min(i+1, n-1)`), NUNCA al inicio del trozo; los acumuladores (UV, ancho, presión suavizada) se siembran del **nudo anterior real**, no de su valor inicial; y el material se asigna a **cada** sección, no solo a la 0.

    ✅ **Y para que la costura sea invisible hace falta que el nudo compartido se calcule igual de los dos lados**, o sea que **no sea el último** del trozo viejo: cerrar el trozo viejo **después** de agregar el nudo siguiente (`AddKnot → Rebuild → Push`), y arrancar el nuevo en `Length - 2`. El quad sobrante del viejo queda degenerado (mismo punto) y no se ve.

386. 🔴🔴🔴 **CORRIGE §385 — Tilt Brush NO TROCEA. El troceado era invención propia, y los tres bugs eran hijos de él, no del algoritmo.** Beltrán lo cortó en seco: *"Arreglalo como lo solucionan en TiltBrush, no inventando"*. El código lo dice sin ambigüedad:

    ```csharp
    ApplyChangesToVisuals() {
        ControlPointsChanged(m_FirstChangedControlPoint);  // regenera desde el 1er nudo CAMBIADO
        m_geometry.CopyToMesh(mesh);                       // UNA malla, el pool entero
    }
    ShouldCurrentLineEnd() => IsOutOfVerts() || NumVerts > m_SoftVertexLimit;  // 9000
    ```

    **Una sola malla por trazo.** El límite de 9000 verts **no es un corte interno: TERMINA el trazo** y el usuario empieza otro. Nunca hay costura porque nunca hay dos pedazos. Yo había metido trozos de 64 nudos como optimización, y de ahí salieron **los tres bugs en fila**: secciones sin material (negro), kernel de suavizado asimétrico en la costura (hueco), y la tangente del nudo costura congelada con un vecino degenerado (el corte que persistió).

    ✅ **El arreglo fue BORRAR la optimización**, no parchearla: `ChunkKnots` 64 → **4500**, que es exactamente el número de ellos (9000 verts ÷ 2 por nudo). Un humano no llega ahí en un trazo.

    💡 **La lección de proceso, que vale más que el bug:** cuando se replica un sistema probado, **una optimización propia agregada encima deja de ser una réplica**. Yo gasté dos viajes al visor y dos hipótesis parchando un artefacto de MI diseño, creíndome que depuraba el de ellos. Antes de optimizar algo que se está copiando: **verificar si el original lo hace, y si no lo hace, preguntarse por qué no.**

    ⚠ Pendiente que esto deja: sin trozos, cada cuadro regenera el trazo ENTERO. TB no paga eso porque regenera solo desde `m_FirstChangedControlPoint` (en nuestro caso, los últimos ~3 nudos son los únicos cuya geometría puede cambiar). Si los trazos largos empiezan a tironear, **ese** es el arreglo fiel — no volver a trocear.

387. 🔴🔴 **Un gate de input por EVENTOS se cuelga; por ESTADO CONSULTADO no puede.** Síntoma (2026-09-25, `BPC_TBTool_NC`): *"a veces no reconoce que solté el botón y sigue dibujando una línea suave"*. Causa: el trazo arrancaba con `Started` y terminaba con `Completed` de un Input Action booleano. Si ese `Completed` se pierde — y había motivo, **dos IMC agregados con la MISMA prioridad compitiendo por el mismo gatillo** — el trazo queda encendido para siempre. Y la presión, que llegaba por el evento `Triggered` del eje, **se congela en el último valor no nulo** (de ahí la "línea suave").

    ✅ **Tilt Brush no usa eventos:** `SketchControlsScript` hace `bool bInputActive = InputManager.GetCommand(SketchCommands.Activate)` **cada cuadro**. Es estado, no evento: si el valor bajó, el trazo termina, haya llegado o no una notificación. La receta portada: en el Tick, **consultar** el valor del eje con el nodo puro `Input|EnhancedActionValues|IA_<accion>` (sin entradas, devuelve Float) y decidir con un umbral. Un gate por estado es idempotente y no tiene estado colgado posible.

    ⚠ Y de paso: el `ActionValue` de un Axis1D **es Float** en el propio nodo de evento; el pin `then` de un nodo de evento está en el **índice 1**, no 0 (el 0 es `OutputDelegate`).

388. 🔴🔴🔴 **REINCIDENCIA — `AddMappingContext` con los defaults del motor suprime el input.** Ya estaba escrito en `CLAUDE.md` §7.b (costó un día en Breath) y en el tracker de `BPC_DrawTool_NC`, **y lo cité al documentar `BP_TBPawn` sin aplicarlo**: los dos `AddMappingContext` quedaron con `bIgnoreAllPressedKeysUntilRelease=True` + `bForceImmediately=False`. Van en **`False` + `True`**, con `Priority=1000`.

    💡 **Lo que hay que cambiar del proceso, no del código:** citar una regla en la documentación **no** es aplicarla. Cuando un tracker diga "esto va con estos valores", el paso siguiente obligatorio es **leer el valor efectivo** (`read_graph_dsl` del grafo muestra el struct de Options entero). Es [[soul-charger-declarado-no-aplicado]] otra vez, en su forma más tonta: declarado por mí mismo, media hora antes, y no aplicado.

383. 🔴🔴 **"El evento contestó" NO es "la app sigue viva": son dos chequeos distintos, y confundirlos hace que el banco mida una app MUERTA y devuelva números plausibles.** El banco de PerfMode usaba como control positivo el eco del `PrintString` del Blueprint (`ke * PerfN` → "PERF: modo N" en logcat) y con eso se daba por satisfecho. Pero el eco solo prueba que **el evento llegó y se ejecutó**; no dice nada de lo que pasa en el cuadro siguiente. El 2026-09-25 el modo 8 imprimía su eco **y a los segundos colgaba la app**, así que el script anunciaba "modo 8 responde: OK" y seguía adelante grabando CSVs de una app congelada. ✅ **La prueba de vida correcta es un segundo chequeo, después de esperar, y por otro canal**: mandar una cvar de solo lectura por consola (`am broadcast ... 'xr.OpenXRFBFoveationLevel'`) y exigir el eco `xr.OpenXRFBFoveationLevel = "0"` en el log. Si el game thread está bloqueado, no contesta. 🔴 Y el canal tiene que ser **distinto del sospechoso**: si lo que se está probando es `ke`, no se puede usar `ke` como probe — se confunde el instrumento con el objeto medido. Implementado en `quest_modo.ps1` (eco del evento **y** prueba de vida, reportados por separado) y en `quest_culpable.ps1`. Ver [[debugging-instrumento-sin-validar]].

384. ⏸️ **Un cuelgue de app en Quest se diagnostica por el ESTADO DEL HILO, y eso separa tres causas que a ojo son idénticas.** Síntoma común: imagen congelada, **sonido siguiendo** (otro hilo), proceso vivo. Los tres casos y cómo distinguirlos con `adb shell "ps -T -p <pid> -o TID,STAT,TIME,CMD"`:
    - **Game thread en `S` con poco TIME acumulado** = BLOQUEADO esperando algo (deadlock). Con la GPU en 0 de presión (`crcs: AppPressureSubscriber: ... gpu=0.0`). **No es un shader lento.**
    - **Render thread en `R` y GPU al 100 %** = shader/carga. Ahí sí se mira el material.
    - **`APP_CMD_LOST_FOCUS` → `APP_CMD_PAUSE` → `SuspendApp_EventThread -> ERROR: backgrounding callback, not responded in timely manner` + `Blocking renderer on suspended window`** = la app pasó a segundo plano y **el camino de suspensión de UE en Android se deadlockea**. Dispara esto: sacarse el casco (sensor de proximidad), y **lanzar la app por intent** (`am force-stop` + `am start`) en vez de abrirla desde la biblioteca del visor.
    🔴 **Corolario de proceso:** el `ensure` de `xrLocateHandJointsEXT` / `XR_ERROR_TIME_INVALID` al final del log **es síntoma, no causa** — el rastreo de manos recibe un tiempo de display inválido *porque* el bucle de cuadro ya se paró. Perseguirlo es perseguir el humo. Y el log del motor en el device (`.../Saved/Logs/VR_Test.log`, más los `-backup-` de cada arranque) permite **contar** una anomalía a través de las sesiones del día: si aparece solo en los builds nuevos, entró con el cambio; si aparece en todos, es ruido crónico.

385. 🔬 **Para medir en visor, el casco va PRIMERO y la app se abre A MANO desde la biblioteca.** Dos causas de cuelgue eliminadas de una: (a) con el visor fuera de la cabeza la Quest suspende la app en segundos → deadlock de §384; (b) lanzarla con `am start` la deja sin una sesión de VR bien establecida → el mismo deadlock. Los scripts del banco (`quest_perfmodes.ps1`, `quest_sesion_larga.ps1`, `quest_bisect_poda.ps1`) ahora **se enganchan a la app que ya corre** y solo la lanzan con `-Lanzar` explícito, e imprimen el pedido del casco **antes** de tocar nada. ⚠ ADB por Wi-Fi sería la solución limpia (saca el cable y sus renegociaciones del medio), pero **no siempre está disponible**: el 2026-09-25 la PC (`172.16.2.75/24`) y el visor (`172.16.2.59/24`) estaban en la misma subred y el ping no pasaba → esa red aísla clientes. Se verifica con un `Test-Connection` antes de invertir tiempo en `adb tcpip`.

389. TERCER CRASH por lo mismo - rompi mi propio gotcha 383 el mismo dia que lo escribi. Construi un material dentro de `execute_tool_script`; el `recompile` fallo (*"Arithmetic between types float4 and float3 are undefined"*), disparo el Undo de las 7 expresiones y mato el editor. Callstack identico al del primero: `ACCESS_VIOLATION 0x430`, `MaterialEditor` x3, desde `PythonScriptPlugin`.

    OK **Procedimiento definitivo para materiales: UNA LLAMADA DIRECTA POR NODO.** `create_material` -> `set_properties` -> un `add_expression` por nodo -> un `set_properties` por parametro -> un `connect_expressions` por cable -> `connect_to_output` -> `recompile` -> `save_assets`. Son ~20 llamadas y es tedioso, pero **cada fallo es atomico**: sin Undo multi-paso, sin crash. Construido asi `M_TB_Additive` a la primera.

    OK **Y reducir la superficie: pocos MAESTROS + muchas INSTANCIAS.** Los pinceles se agrupan por modo de mezcla (aditivo / masked / lit). Un maestro por modo, hecho a mano una sola vez; cada pincel es un `MaterialInstanceConstant` cuyos parametros son **escrituras de propiedad** (`MaterialInstanceTools.set_{scalar,texture}_parameter`), que no tocan grafo y no pueden crashear.

390. Tres normalizaciones de nombre del DSL, ya confirmadas las tres: el guion bajo **se cae** en variables (`K_Pos` -> `GetKPos`) **y tambien en funciones** (`Brush_Light` -> `CallFunction|BrushLight`); la `b` inicial se cae en bools; y la **categoria** de una variable entra en el id. Ante *"X does not exist"* con la funcion claramente creada: `find_node_types(graph, "CallFunction|<PrefijoSinGuion>", [])`. Ademas, una funcion recien agregada **no es invocable hasta recompilar**.

391. `MaterialExpressionVertexColor` devuelve float3 (RGB) en su salida sin nombre, no float4. Multiplicarlo contra `TextureSample."RGBA"` (float4) **no compila**. Va contra `"RGB"`. El alfa se toma aparte por la salida `"A"` de cada uno.

392. 🔴🔴 **El `MotionSource` de un `MotionControllerComponent` NO lo define su nombre — nace en `Left` y hay que ponerlo a mano.** Costo un viaje al visor con el sintoma total *"ya no aparece la paleta ni dibuja"*. En `BP_TBPawn` yo habia creado `MC_RGrip` y `MC_LGrip` y **nunca les escribi el `MotionSource`**: los dos quedaron en el default `Left`. `BP_TBDrawRig` clasifica los mandos comparando ese string contra `RightAim` / `LeftGrip` (la convencion del VRPawn del VRTemplate, verificada: `LeftGrip`, `RightGrip`, `LeftAim`, `RightAim`), asi que la mano izquierda **nunca se encontraba** -> el gate de instalacion no pasaba -> no se instalaba el input, ni la herramienta, ni la paleta. Y de paso `MC_RGrip` estaba **siguiendo la mano izquierda**, un bug real e independiente.
    - Es el caso de libro de "declarado != aplicado": el **nombre** del componente es una declaracion mia; el `MotionSource` es el valor efectivo. **Leelo, no lo deduzcas del nombre.**
    - 🔴 **Y el arreglo en el CDO NO llega a la instancia ya colocada**: despues de escribir la plantilla (`BP_X_C:MC_LGrip_GEN_VARIABLE`) y recompilar, la instancia del nivel **seguia en `Left`** — lo tenia serializado como override propio. Hay que escribir **las dos**, y **releer las dos** (ver "Lo de la INSTANCIA le gana al Blueprint").
    - ✅ Verificado sin visor con el ciclo autonomo: dos `PrintString` temporales (uno al final de `DoInstall`, otro al final de `MountPalette`) + `StartPIE({playMode:"PlayMode_InViewPort"})` + `GetLogEntries(pattern, category:"LogBlueprintUserMessages")`. **`GetLogEntries` exige `category` explicita**; sin ella contesta *"Log category 'LogsToolset' not found"*. Los mandos existen como componentes en PIE aunque no haya casco, asi que la cadena de deteccion e instalacion se verifica entera de escritorio. Los prints se borran despues.

393. 🔴 **Un gate que exige TODO encontrado convierte cualquier falta en "nada funciona".** El rig tenia `IsValid(RightAim) -> IsValid(LeftGrip) -> DoInstall`: al faltar el mando izquierdo se perdia **tambien el dibujo**, y el sintoma no señalaba nada. Arreglado moviendo el chequeo a donde importa: `IsValid(RightAim) -> DoInstall`, y `MountPalette` se protege sola con `IsValid(LeftGrip)`. Ahora una mano izquierda mal configurada cuesta **solo la paleta** y el sintoma **nombra la pieza**. El gate conjunto tampoco compraba nada: los cuatro componentes del pawn llegan juntos en el mismo `GetComponentsByClass`, no hay carrera entre ellos. **Regla: cada dependencia se chequea en el punto donde se usa, no en un gate global.**

394. 🔴🔴🔴 **En el DSL el PRIMER argumento es el TARGET — y con funciones de libreria cuyo primer parametro es un objeto compatible, eso compila perfecto y no hace NADA.** El caso (2026-09-26, `BP_TBDrawRig`): escribi `(Input|EnableInput _playercontroller)` creyendo que pasaba el controller como parametro. El nodo quedo con el **PlayerController en el pin `self`** y el parametro `PlayerController` **vacio**: `EnableInput(target = el controller, controller = null)`. Un `APlayerController` **es** un `AActor`, asi que el tipo calzo, el Blueprint **compilo sin una sola advertencia** y el rig nunca registro su InputComponent → sus eventos de Enhanced Input no existian para el motor. Sintoma: la paleta (que selecciona por proximidad) andaba y el dibujo no.
    - 🔴 **Y mi verificacion en PIE no lo agarro**: el `PrintString` al final de `InstallInput` imprimia bien, porque la llamada **se ejecutaba** — solo que sobre el objeto equivocado. **Verifique que la funcion CORRIA, no que hiciera EFECTO.** Una sonda de ejecucion no es una sonda de efecto.
    - ✅ Como se detecta barato: `get_node_infos` sobre el nodo y **mirar quien alimenta `self`**. Si el pin del parametro aparece con `connected_pins: []`, esta sin conectar por mas que el DSL lea razonable. Aplica a toda funcion con target implicito: `EnableInput`, `AttachComponentToComponent`, `SetVisibility`…
    - ⚠ Relacionado: **conectar a un pin de DATOS ya conectado NO reemplaza siempre** (contra lo que decia `toolsets.md`): el `self` quedo con **dos** conexiones y hubo que `break_pins` la vieja a mano. Releer el nodo despues de cada conexion.

395. ⚠ **CORREGIDO el 2026-09-26 — ver la correccion al pie de este punto: `IA_Hand_IndexCurl_*` SI es el gatillo (su eje analogico).** El diagnostico original decia: *`IA_Hand_IndexCurl_*` NO es el gatillo, es la curvatura capacitiva del dedo, y nunca llega a cero con el mando en la mano.* Sintoma reportado: *"a veces suelto el trigger pero se queda pegado dibujando delgado"*. El log lo dijo sin ambiguedad: **`Triggered` de esa accion dispara en TODOS los cuadros, incluso antes de apretar**. Con el dedo apoyado en el gatillo el valor se queda en ~0,1-0,3, o sea **por encima** del umbral 0,08 donde yo habia colgado el corte del trazo → el trazo no termina, y sale fino porque la presion es baja. "A veces" = cuando levantaba el dedo del todo, si cortaba.
    - 🔴 **El gate de presion era invencion mia** (lo habia agregado para tapar los eventos perdidos de §387). Tilt Brush no corta por presion: consulta **`Activate` (el gatillo)** en cada cuadro. Arreglado a su modelo: `Rig.Tick → GateStop(Get IA_Shoot_Right)` y `GateStop(Held) = if (not Held) and bDrawing → Release`. **El gatillo decide si el trazo vive; el dedo solo modula el grosor.**
    - El nodo para consultar estado es **`Input|EnhancedActionValues|<IA_...>`** (puro, devuelve el valor actual). Existe para toda accion mapeada y es la forma de hacer polling al estilo TB en vez de depender de eventos.
    - ⚠ Al pasarle el estado por parametro hay que **recompilar el BP de la funcion ANTES** de que el nodo que la llama muestre el pin nuevo.
    - 🔴 **CORRECCION (2026-09-26, la misma sesion que lo escribio mal).** `IA_Hand_IndexCurl_Right` esta mapeada a **`OculusTouch_Right_Trigger_Axis`**: es el **eje analogico del gatillo**, no un sensor capacitivo. La capacitiva es **`IA_Hand_Point_*` (`Trigger_Touch`)**. Se confirma **por eliminacion** sobre `IMC_Hands`: sus cuatro acciones por mano consumen exactamente cuatro teclas — `Grip_Axis`(Grasp), `Thumbstick/A/B/X/Y_Touch`(ThumbUp), `Trigger_Axis`(IndexCurl), `Trigger_Touch`(Point).
      - Lo observado seguia siendo cierto y el arreglo del gate tambien: el eje del gatillo **no vuelve a 0** con el dedo apoyado, asi que colgar el CORTE del trazo de el estaba mal igual. Lo que estaba mal era la EXPLICACION, y una explicacion equivocada manda mal la proxima decision — por eso la presion se quedo en ese pin creyendo que era "el dedo".
      - `IA_Shoot_Right` (el gate) esta en `IMC_Weapon_Right` sobre **`Trigger_Click`**, y su `ValueType` es **Boolean** → como fuente de presion daria 0/1. **El gatillo de la Quest da presion analogica; que llegue analogica o booleana lo decide el `ValueType` de la accion y la tecla mapeada.**
      - 🔴 **`ObjectTools.get_properties(<IMC>, ["Mappings"])` devuelve `[]` aunque el IMC tenga mapeos** (verificado en dos IMC que funcionan). El lector no sirve para eso: leer las teclas con `grep -a` sobre el `.uasset`.

396. 🔴🔴🔴 **Agregar componentes a un Blueprint NO llega a las instancias ya colocadas — y despues esa instancia RECHAZA las escrituras devolviendo `true`.** Medido tres veces el 2026-09-26 (dos en `BP_TBPawn`, una en `BP_TBDrawRig`). El actor colocado crea los componentes nuevos con los **valores por defecto** y los trata como override propio: `staticMesh: None`, transform en cero, `overrideMaterials: []`, aunque la plantilla del BP los tenga puestos. Recompilar no lo arregla.
    - 🔬 **Como se confirma en dos llamadas:** colocar una instancia NUEVA del mismo BP y leerle los componentes. Si la nueva hereda bien y la vieja no, el Blueprint esta sano y la instancia esta desfasada.
    - 🔴 **Parchear la instancia vieja puede no funcionar:** de seis propiedades escritas, entro **solo `staticMesh`**; las otras cinco devolvieron `true` y no se aplicaron. **Releer siempre** (es el unico modo de verlo). Cuando eso pasa, la salida es **reemplazar el actor**, no insistir.
    - 👉 Consecuencia para transplantar una mecanica: si el BP gana componentes despues de colocarlo, **hay que volver a colocarlo**. Recompilar no alcanza.

397. 🔴🔴 **Un cubo NO sirve como lienzo para probar un World Position Offset.** El 2026-09-26, con `M_BlobMesh_SC` invisible en la Quest y visible en PC, arme un biseccion de cuatro variantes del material y las puse sobre **cubos**: A (color plano) se veia, B (+WPO) se veia **solo por el ojo derecho**, C (+normal por interpolador) se veia, D (los dos) no. Parecia que el WPO era el culpable. **No prueba nada.** El WPO de este material manda cada vertice a la superficie de la cadena de gotas: un cubo tiene **8 vertices**, asi que el resultado es una lasca degenerada y diminuta en cualquier lado — y con el material a **una sola cara**, una lasca casi de canto se ve desde un ojo y se descarta por backface culling desde el otro. ⚠ **Y la explicacion que le puse al "solo el ojo derecho" TAMBIEN estaba mal**: dije que era una lasca casi de canto vista desde dos angulos. No: repetido despues sobre la **malla real de 2.037 vertices** el mismo ojo unico volvio a aparecer, asi que era el sintoma de verdad del miscompile (ver 399). Moraleja doble: el test invalido no solo no probaba nada, ademas me dio una explicacion falsa que sonaba bien.
    - ✅ **La regla: el control de un WPO va sobre la MALLA REAL.** Lo unico que la tanda de cubos dejo en firme es lo que NO depende de la posicion: el setup del material (unlit + translucido + una cara) compila y dibuja en la Quest, y `Custom_1` a traves del `VertexInterpolator` tambien.
    - 🔬 Y la escalera que si sirve son controles **independientes** sobre la malla real, cada uno aislando una pieza: tubo plano sin WPO / tubo con WPO **constante** / tubo con WPO desde un Custom que usa la posicion local / el material real. Un control que sube 300 uu se **autodemuestra**: si aparece levantado, el WPO llego.

398. 🔴 **El `LocalPos` del material venia de `TransformPosition(WorldPosition, World->Local)`, no del nodo `LocalPosition`** — y la estacion esta a **3.629 m del origen** (X ~ 362.790 uu). Es un ida y vuelta por coordenadas de mundo dentro del **vertex shader movil**, justo donde la precision se paga: en `half` (max 65.504) ese X ni siquiera entra. El nodo `LocalPosition` da el mismo valor en PC sin salir del espacio local. Cuando un material anda en PC y no en movil, **mira si algo pasa por coordenadas de mundo absolutas**.
    - ⚠ En el mismo grafo quedaba `PerfMode` (una `CollectionParameter`, o sea un MPC) **conectada al Custom del vertex shader aunque el codigo no la usa**. Un pin conectado se compila igual. El gate del banco ya se habia mudado al pixel shader **por** ese motivo, pero el pin quedo. Para sacar una entrada de un Custom sin romperlo (UE exige que TODAS esten conectadas), lo barato es **alimentarla con un `Constant`** en vez de pelear con el arreglo `inputs`.

399. 🔴 **"El color se ve distinto al que elegi" casi nunca es el valor: es el tonemapper y el brillo con que lo mostraba lo anterior.** Beltran reporto que el naranja de los pinceles TB se veia **rojo**. El valor era **bit a bit** el aprobado (`Colors[0]` de `BPC_DrawTool_NC` = (1, 0.26356, 0.032371)), el material era `tex.RGB * VC` y la textura tenia **RGB blanco puro** — todo inocente. Lo que cambio fueron dos cosas, y conviene **medirlas en vez de discutirlas**:
    - **El tonemapper filmico de Unreal**: lineal (1, 0.2636, 0.0324) se muestra como **(213, 124, 40)**, no como los **(255, 140, 50)** del color picker. Mas oscuro y mas saturado. Es el clasico "mi emisivo naranja se ve rojo".
    - **Lo anterior no mostraba el color puro**: la cinta aprobada tenia `GradAmount=1` mezclandolo con `GradColors=(0.97,0.96,1)` ≈ blanco → el ojo tenia guardado un **(254, 170, 131)**. Contra eso, el color plano y saturado se lee rojo.
    - 🔑 **El metodo**: antes de tocar el valor, (1) leer el valor efectivo y compararlo con la fuente aprobada, (2) confirmar que el material y la textura no lo tinten, (3) **calcular** el paso por el tonemapper (fit ACES, 20 lineas de Python) y comparar con lo que muestra el picker. Recien ahi se sabe si hay que cambiar el color, el brillo, o nada.
    - ⚠⚠ **Y la trampa de calibrar color en el preview de PC:** si el proyecto tiene **`r.MobileHDR=False`** (el caso de Neural Canvas), el APK en la Quest corre **sin tonemapper** y muestra el color exacto. O sea que las dos rutas NO coinciden: "arreglar" el color mirando el VR Preview de PC lo deja mal en device. Antes de calibrar color, **mirar `r.MobileHDR` y saber en cual de las dos rutas se esta mirando**.

400. ⚠ **El toolset de PIE/cvars se llama `EditorToolset.EditorAppToolset`**, no `editor_toolset.toolsets.editor_app.EditorAppToolset` (que es el patron de casi todos los demas). Adivinarlo cuesta un `list_toolsets`, que son ~8k tokens. Ahi viven `IsPIERunning`, `StartPIE`, `StopPIE` y los cvars.

399. 🔴🔴🔴 **En el VERTEX SHADER de la Quest, un arreglo local con INDICE DINAMICO se miscompila: la malla se dibuja en un ojo y en el otro no.** Medido el 2026-09-26 con una escalera de controles sobre la malla real. `M_BlobMesh_SC` se veia perfecto en el editor **en Vulkan Mobile Preview** (o sea: el shader movil ya compilaba y dibujaba bien en PC) y **no se veia en el visor**, sin error de compilacion ni advertencia en el log del device.
    - **La causa**: los bucles tenian el tope en una variable (`ND`/`NP`/`NA`, derivadas del parametro `NBlobs`) y se leia `P[]`/`R[]`/`TG[]`/`ACC[]` con indice dinamico. Con tope variable el compilador **no puede desenrollar**, y los arreglos caen en *indexable temp* (memoria en vez de registros). El Vulkan de escritorio lo resuelve bien; el Adreno de la Quest no. Y como el vertex shader corre **por vista** bajo multi-view, la corrupcion toca una vista y la otra no → **el sintoma es un ojo**.
    - ✅ **El arreglo**: tope de bucle **constante** (8 o 9) y un **peso `A[j]` de 0 o 1** que decide quien participa, en vez de recortar el bucle. Para un smooth-min el peso 0 es la identidad **exacta** (`hh = saturate(...) * A[j]` → `d = lerp(d, ds, 0) - k*0*(1-0) = d`), asi que el resultado no cambia. Las lecturas del tipo `P[N-1]` se reemplazan por una seleccion desenrollada (`[unroll] for i<8: Pend = lerp(Pend, P[i], step(abs(i-(N-1)), 0.5))`). Silueta verificada contra el original en el viewport, y **el reescrito se ve en los dos ojos**.
    - 📈 La gravedad **escala con el tamaño del shader**: con un solo Custom asi en el vertex shader se perdia **un ojo**; con dos (el desplazamiento **y** la normal por `VertexInterpolator`) se perdian **los dos**. O sea que "se ve en un ojo" es la version leve del mismo bug.

400. 🔴 **Un guardia de NaN con un tope absurdo convierte "calcula mal" en "no se ve", y eso cuesta viajes al visor.** El material tenia `if (!(dot(off,off) < 1.0e12)) return 0;` — 1e12 sobre el CUADRADO es **un millon de unidades**. Cualquier basura finita pasaba el guardia, los vertices se iban fuera de cuadro y el sintoma era "la malla no existe", que no señala nada. Con un **tope fisico** (`off *= min(len, 400) / len`) la misma basura se ve como una malla **deforme**: el sintoma pasa a decir "el shader corre y la cuenta esta mal", que es la mitad del diagnostico. **El limite de un guardia se elige con la escala del objeto, no con el limite del tipo.**

401. 🔴 **Un `Gain` copiado de Tilt Brush no se puede copiar sin su HDR.** `MI_TB_Light` tenia `Gain = 180.03`, que es literal de ellos: `Light.mat` trae `_EmissionGain: 0.45` y su `Bloom.shader` hace `2 * exp(gain*10)` = 180,03. **Pero ese 180 vive en un pipeline HDR con bloom**: los valores por encima de 1 se vuelven halo y el tonemapper los devuelve. En Neural Canvas hay **`r.MobileHDR=False`** → no hay rango por encima de 1: **todo pixel con alfa de textura > 1/180 = 0,0056 clipea a blanco**, y el pincel sale blanco puro en vez de una linea de luz de color. Reportado por Beltran como *"Light sale blanco"*.
    - ✅ La regla para elegir el reemplazo en LDR: con ganancia `G`, la fraccion del trazo que se quema es donde `alfa > 1/G`. `G=4` quema el cuarto mas denso y deja el resto como caida de color → nucleo blanco con halo naranja, que es como se lee una luz sin HDR. `G=1` deja el color puro sin nucleo blanco.
    - 🔑 Generalizando: **antes de portar una constante, mirar en que espacio numerico vive**. `EmissionGain`, `bloomColor`, emisivos > 1 y cualquier cosa pensada para bloom son valores HDR; en un proyecto con MobileHDR apagado hay que reescalarlos, no copiarlos.

402. 🔬 **Una captura del visor es un INSTRUMENTO: medile los pixeles en vez de mirarla.** Beltran mando dos fotos diciendo *"sale con un color mas rojizo que naranjo"*. Midiendo con PIL: el nucleo de los DOS trazos daba exactamente **(255, 140, 50)** — o sea el naranja elegido, exacto. Lo que los diferenciaba era la COBERTURA: el trazo "rojizo" tenia **8 px de ancho y solo 0,1% de sus pixeles a color pleno** (opacidad mediana 0,23), contra 58 px y 95% del otro. No era un problema de color: el pincel salia tan fino que se mezclaba con el fondo azul, y naranja al 23% sobre azul **es** rosa sucio.
    - 🔧 **La receta** (20 lineas): sacar el color de fondo como el mas frecuente, proyectar cada pixel sobre la recta `fondo -> color esperado` para obtener su alfa efectivo, descartar por residuo los que no caen en esa recta, y mirar el **histograma** del alfa y el ancho en px. Separa en un paso "el color esta mal" de "el color esta bien pero casi no pinta".
    - ✅ De yapa dio otra respuesta gratis: que el nucleo diera (255,140,50) y no (213,124,40) **prueba que ese preview no estaba aplicando tonemapper** — confirmando `r.MobileHDR=False` en la ruta Android, sin tener que preguntarlo.

403. 🔴🔴 **Verifiqué la rama que NO corre: el pincel tenía un flag que elegía otra función.** Petal no separaba sus pétalos. Yo había ido al código de Tilt Brush y verificado que "el normal del vértice **es** la dirección radial, así que sumar el empuje al radio es equivalente" — y era cierto... en `MakeClosedCircleSoftEdges`. **Pero el prefab de Petal trae `m_HardEdges: 1`, o sea que corre `MakeClosedCircleHardEdges`**, que emite **dos vértices coincidentes por ángulo con normales DISTINTAS** (la de la cara anterior y la de la siguiente). Como el empuje del pétalo va `offset = m_Normals[vert] * ...`, los dos vértices que comparten una arista **se van en direcciones diferentes y el tubo se rasga en pétalos**. Con normales suaves compartidas el anillo nunca se abre: sale una trompeta.
    - 🔑 **`m_HardEdges` no era un detalle de sombreado: era el mecanismo de la forma.** Un flag booleano del prefab que yo había anotado como desviación menor ("con material unlit no se nota") decidía la geometría entera.
    - ✅ **La regla**: cuando una estructura elige entre implementaciones por un flag (`m_HardEdges`, `m_EndCaps`, `UVStyle`, `ShapeModifier`), **leer el valor del flag ANTES de leer el código**, y verificar la rama que ese valor selecciona. Verificar la otra da una conclusión correcta sobre código que no se ejecuta — que es peor que no verificar, porque queda escrita en el tracker como si fuera verdad. Es la misma familia que [[leer-la-funcion-que-ya-anda]].
    - 📐 De paso: su `MakeClosedCircleHardEdges` usa el vértice duplicado para cerrar los UV del anillo. Nuestro emisor viejo cerraba con `j2 = (j==N-1) ? 0 : j+1` sobre vértices compartidos → la última cara tenía la UV invertida (una costura). Al portar las aristas duras el problema desapareció solo.
