...
61. 🔴🔴 **`(CallFunction|X ...)` puede resolver a una función del MISMO NOMBRE de OTRO Blueprint.** Pasó dos veces el 2026-08-15 en `BP_ConstExplorer`: `(CallFunction|HoverFeedback)` compiló como **`Class|BPMenuButton|HoverFeedback`** y `(CallFunction|ResolveHover)` como **`Class|BPAimBeam|ResolveHover`** — llamadas a otra clase, en verde, sin una sola advertencia. Antes había pasado lo mismo con `SetRow`, que se fue a `Layout|GridSlot|SetRow`. **Regla: ponerle a las funciones propias un nombre ÚNICO en todo el proyecto** (prefijo del BP: `ExploreResolve`, `SetResultRow`, `CacheResultsBio`) **y releer el grafo después de escribir** — el `read_graph_dsl` muestra a qué clase resolvió cada llamada, y ahí se ve.
