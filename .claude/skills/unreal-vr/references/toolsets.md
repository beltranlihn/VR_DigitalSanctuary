...
🔴 **`EditorAppToolset` y `LogsToolset` NO siguen el patron `editor_toolset.toolsets.X`**: su path completo es **`EditorToolset.EditorAppToolset`** y **`EditorToolset.LogsToolset`** (verificado 2026-09-26). Ahi viven `IsPIERunning`/`StartPIE`/`StopPIE`/cvars y `GetLogEntries`. Equivocarse cuesta un `list_toolsets` = ~8k tokens.
