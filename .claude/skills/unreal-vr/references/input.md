...
- 🔴 **El array `Mappings` de un IMC NO se puede escribir por MCP** (structs `FEnhancedActionKeyMapping` — `get_properties` sobre `Mappings` devuelve `[]` aunque el IMC tenga mapeos; verificado 2026-07-30 en `IMC_Weapon_Right`). O sea: **no puedo crear un IMC nuevo y mapearle teclas por MCP**. Salidas: (a) **crear el IMC/mapeos a mano en el editor** (la tablita visual, 30 s), o (b) 🔑 **reusar una acción que YA está mapeada y activa** — casi siempre la mejor. Corolario: **`IA_Hand_IndexCurl_Left/Right`** (XRFramework, **Axis1D**, en `IMC_Hands` que el pawn agrega) **ES la presión analógica del gatillo también en modo mandos** (el framework mapea el curl del índice al eje del trigger para animar las manos). Para presión analógica del gatillo sin plomería nueva: consumir el evento `IA_Hand_IndexCurl_*.Triggered` y leer su `Action Value` (float). Usado así en `BP_BrushTool` (stage Movement).
