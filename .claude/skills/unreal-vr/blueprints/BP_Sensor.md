...
- ✅ **Guion §7 COMPLETO (2026-08-13)**: si nadie lo toma en `AutoAssignTime` (12 s, CDO de [[BP_Stage_Hall]]), el Hall llama **`ForceTakeRight()`** → `ForceTakeBody` (bIsRight=true, bTookRight=true, CacheHand) → `TryForcedAttach` (IsValid HandRef → `Take()`; en Simulate sin pawn, log y sigue). El poll `CheckTaken` del Hall detecta el bTaken y hace el resto solo (segundo sensor + persistencia). Log: `SENSOR: asignacion automatica a la DERECHA (guion 7)`. 💡 Bonus: el run automático ahora ejercita TODO el camino de la mano hábil sin visor.
