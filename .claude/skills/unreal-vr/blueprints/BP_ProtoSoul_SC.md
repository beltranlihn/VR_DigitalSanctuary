...
🚩 **Y hay un SEGUNDO array que también había que extender: `RingReveal`.** Con 4 entradas, `PushRingMat(4)` lee el índice 4 fuera de rango → `Reveal = 0` → **el anillo existe, con su geometría y su color, pero el material lo dibuja invisible.** El síntoma es engañoso: parece que el componente no se creó. ✅ CDO a `[1.05 × 5]`.
⚠ `RingReveal` vive en *Z - Estado interno* y **no es instance-editable**: `set_properties` sobre las instancias lo rechaza. No hace falta — al no tener override propio, las cinco heredan el del CDO (verificado leyendo una).
✅ **Verificado visualmente en el viewport**: los cinco anillos dibujando, el verde por fuera del naranja en las cinco amebas. Compila y corre sin `Accessed None`.
...
| **`HideRings()`** (BeginPlay) | `DrawIndex = -1` y los 5 `RingReveal` a 0. Los anillos **nacen visibles en el editor y ocultos en juego**. ⚠ **Es un CORTE EN DURO, no una animación** — sirve para el arranque, pero **no para apagar anillos a la vista**: llamarla al cerrar una lámina hace que los anillos desaparezcan de golpe. Para eso alcanza con `Disappear()`, que ahora se los lleva por escala (ver `ApplyRingScale`). |
...
| **`ApplyRingScale()`** (Tick) | `RingRoot.escala = (Size / RingSizeRef) × **`Ease(EaseInOut, AppearT, 0, 1)`** y, **sólo cuando cambia** (ese gate sigue mirando el cociente sin el ease, para no re-empujar material cada frame), re-empuja `ScaleComp` a los 5. 🆕 **El factor `AppearT` es lo que hace que los anillos NAZCAN Y MUERAN CON EL CUERPO**: la misma curva que `StepAppear` le aplica al `Body`, así que crecen y se achican juntos. En reposo `AppearT = 1` → factor 1 → la escala es la de siempre. |
