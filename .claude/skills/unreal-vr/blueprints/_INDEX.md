...
| 🆕🧪 **BP_SelfTest** | `Core/Debug/` | **Batería de aserciones por log** (`TEST PASS/FAIL/SKIP/SUMMARY`). Permite verificar **funcionamiento sin visor y sin humano**: `StartPIE` → `GetLogEntries("TEST ")` → `StopPIE`. 🔴 **Correr también en `bSimulate:true`** — ya encontró un bug que el PIE normal escondía. 15 aserciones. | 🟢 15 pass en PIE y en Simulate | [✓](BP_SelfTest.md) |
