...
| **BP_StageDirector** | `Core/Flow/` | §9.4/§9.2: el ciclo **completo** precarga invisible → baja la luz + traza el marco → negro → swap + reposiciona el pawn → abre y sube la luz. Encadenado con `SetTimerByFunctionName` sobre custom events (un `Delay` no vale dentro de una función). Loopea sobre `StageNames`. **Reemplaza a `BP_FlowDirector`**, cuyo actor ya se quitó del persistente. 🆕 **2026-08-12: lleva la lista real de 7 SALAS** (Hall + 5 etapas + final) con su tipo en `RoomKinds`, la **duración sale del tipo**, y la última sala **no tiene compuerta** (caso terminal: `CloseOrFinish` → `FinishObra`). Nombra las instancias con un **serial monótono** (con el índice de etapa se repetía y la obra quedaba en negro) y **descarga la sala vieja** bajo negro. Queda 1 decisión autoral pendiente en el tracker. | 🟡 recorrido completo verificado por log, falta visor | [✓](BP_StageDirector.md) |
