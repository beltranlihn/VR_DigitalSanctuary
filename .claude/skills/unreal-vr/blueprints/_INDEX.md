...
| **BP_StageDirector** | `Core/Flow/` | §9.4/§9.2: el ciclo **completo** precarga invisible → baja la luz + traza el marco → negro → swap + reposiciona el pawn → abre y sube la luz. Encadenado con `SetTimerByFunctionName` sobre custom events (un `Delay` no vale dentro de una función). Loopea sobre `StageNames`. **Reemplaza a `BP_FlowDirector`**, cuyo actor ya se quitó del persistente. 🐛 2 bugs conocidos en el tracker. | 🟡 corre y loopea (log en PIE), falta visor | [✓](BP_StageDirector.md) |
| 🆕🧪 **BP_SelfTest** | `Core/Debug/` | **Batería de aserciones por log** (`TEST PASS/FAIL/SKIP/SUMMARY`). Permite verificar **funcionamiento sin visor y sin humano**: `StartPIE` → `GetLogEntries("TEST ")` → `StopPIE`. 🔴 **Correr también en `bSimulate:true`** — ya encontró un bug que el PIE normal escondía. 18 aserciones. | 🟢 18 pass / 0 fail en PIE y en Simulate | [✓](BP_SelfTest.md) |
