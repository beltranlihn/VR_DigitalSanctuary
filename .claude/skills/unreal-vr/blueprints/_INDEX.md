...
| **BP_StageDirector** | `Core/Flow/` | §9.4/§9.2: el ciclo **completo** precarga invisible → baja la luz + traza el marco → negro → swap + reposiciona el pawn → abre y sube la luz. Encadenado con `SetTimerByFunctionName` sobre custom events (un `Delay` no vale dentro de una función). Loopea sobre `StageNames`. **Reemplaza a `BP_FlowDirector`**, cuyo actor ya se quitó del persistente. 🆕 nombra las instancias con un **serial monótono** (con el índice de etapa se repetía y la obra quedaba en negro) y **descarga la sala vieja** bajo negro. Queda 1 decisión autoral pendiente en el tracker. | 🟡 3 ciclos verificados por log, falta visor | [✓](BP_StageDirector.md) |
| 🆕🧪 **BP_SelfTest** | `Core/Debug/` | **Batería de aserciones por log** (`TEST PASS/FAIL/SKIP/SUMMARY`). Permite verificar **funcionamiento sin visor y sin humano**: `StartPIE` → `GetLogEntries("TEST ")` → `StopPIE`. 🔴 **Correr también en `bSimulate:true`** — ya encontró un bug que el PIE normal escondía. 20 aserciones. **Ya cazó 4 bugs**, incluido uno fatal del streaming que dejaba la obra en negro para siempre. | 🟢 20 pass / 0 fail en PIE · 19/0/1 skip en Simulate | [✓](BP_SelfTest.md) |
| **BP_DebugDirector** | `Core/Debug/` | §9.9: dispara `ForceComplete` del director (que cierra por el **mismo camino** que una finalización real, nunca teletransporta). Combo de **dos gatillos sostenidos** + modo **soak** para dejar el ciclo corriendo solo. Suelto en el persistente: se borra y listo, cero código de debug en las etapas. Falta el HUD. | 🟢 ForceComplete verificado · 🟡 combo sin visor | [✓](BP_DebugDirector.md) |
