...
| 🆕 **BP_Journey** | `Core/Movement/` | **El recorrido completo de la obra en UN spline** con 7 paradas que se arrastran en el viewport + `LegTimes` por tramo (con fallback, agregar puntos no rompe). API por *input key*, nunca por distancia (el reparamTable queda viejo al escribir puntos por MCP). | 🟢 caminata verificada en PIE; el director aún no lo usa | [✓](BP_Journey.md) |
