...
| **`BP_MenuButton`** (botones del menú **y el TIMBRE**) | ✅ **Cableado 2026-08-15**: `HoverEdge` detecta el flanco (entrada **y salida** del hover, como pide el guión) → `SBubble` + `HapticHover`; `Fire` → `SSelect` + `HapticSelect`. La mano correcta sale de `bHoverRight`, que se setea entre las dos comprobaciones de mano. 🔔 **Y la VIBRACIÓN CONTINUA del timbre**: `TickHold` → `HoldStart` (una sola vez, por flanco `bWasHolding`) → `SBell` + **`HapticHold(on)`**; se corta en `Fire` y al perder el hover (`HoldStopIfOff`). El timbre es un `BP_MenuButton` con `bHoldByHover=true`, o sea **sin gatillo**, tal como pide la decisión #4. ⬜ Falta el **ring slider visual** 0→100. |
