...
## 🗣️ 2026-08-19 — Alma HABLA: los 33 voice overs viven acá
Pedido de Beltrán: la maqueta de la obra con los VO de Alma como anclas de timing (ver [[BP_Director_Story]]).
- **`VOClips`** (SoundBase[], cat. *I - Voz*, instance-editable): **34 entradas, la entrada `i` es "Voice Over i"** (la 0 y la 6 vacías — el 6 no existe). Así el número que se habla es el índice, sin desfase. Se cargaron desde `Core/Audio/VO/` (⚠ `Voice_Over_7_` y `Voice_OVer_26` tienen el nombre raro del import; el array ya apunta bien).
- **`Speak(Index)`**: corta lo que estuviera sonando (`StopSpeaking`: `ClearTimer` de los TRES timers + `Stop` del `VOComp`), guarda `CurrentVO`, loguea `ALMA: habla VO i`, y — 🆕 2026-08-20 — espera **`VODelayIn`** antes de sonar: timer a `SpeakNow()`, que hace el chequeo del array y `PlayClip(Clip)` → `SpawnSound2D(clip, VOVolume)` + **timer `VODone` a `GetDuration(clip)`**. No se bindea `OnAudioFinished`: la duración del clip es conocida y un timer es un nodo.
- 🆕 **`VODelayIn` / `VODelayOut`** (cat. *I - Voz*, instance-editable; **la instancia está en 1,0 / 1,0**, CDO en 0): el aire ANTES de que la voz entre y DESPUÉS de que termine. `VODone` loguea `termino VO i` al acabar el clip y espera `VODelayOut` antes de disparar `OnVOFinished` (vía `NotifyVO()`). **El evento completo dura `VODelayIn + clip + VODelayOut`** — con 1/1, dos segundos más que el clip. Con 0/0 el comportamiento es idéntico al de antes (el timer no se agenda, se llama directo). ✅ Medido en PIE: VO2 = 5,56 s (1+4,5) y 1,00 s exacto entre `termino` y el paso siguiente; una interrupción (VO5 sobre VO4) no dejó timers colgados.
- **`VODone()`** → loguea `ALMA: termino VO i` y dispara **`OnVOFinished`** (dispatcher, sin params). 🔴 **Índice vacío o fuera del array = `OnVOFinished` al segundo**, con log: un VO que falte nunca traba la obra (regla "cero callejones sin salida").
- **`bStartHidden`** (true en CDO e instancia): `BeginPlay` → `Boot()` → `HideNow()` (`bLeaving`, `AppearT=0`, `Body` a 0) en vez de `Appear`. Alma **existe en el mundo desde el principio pero no se ve** hasta el primer `AppearAt`. Cirugía: el `BeginPlay → Appear` pasó a `BeginPlay → Boot`.
- 2D y head-locked a propósito (no hay spatializer para 5.8; la voz de la guía es el "score hablado"). `VOVolume` es la perilla.
⚠ Un lote de `write_graph_dsl` relanzado duplicó `Speak`/`PlayClip` (24 huérfanos); se limpiaron con `clean_orphans.py` (`identical=true`). Regla en `gotchas.md`.

## 🎤 2026-08-20 — Alma AUDIOREACTIVA a su propia voz
Pedido de Beltrán. El **envelope follower nativo del `AudioComponent`** (attack 10 ms / release 150 ms, seteados en `PlayClip`) alimenta la deformación y el brillo del material mientras habla:
- 🔴 **El bind va ANTES del Play, o el envelope NUNCA dispara** (la causa del "no reaccionó nada" del primer intento): el motor congela `bUpdateSingleEnvelopeValue` en el instante en que el sonido ARRANCA — un `SpawnSound2D` (que auto-reproduce) seguido del `Assign` deja el ActiveSound sordo para siempre. La receta: **`CreateSound2D` (no reproduce, `bAutoDestroy` true) → attack/release → `BindVOEnv` → `Play`**. El control positivo es el log `ALMA: envelope ACTIVO` (dispara una sola vez por sesión, función `EnvSeen`).
- `PlayClip` → `BindVOEnv` (evento) → `AssignOnAudioSingleEnvelopeValue` sobre el `VOComp` recién spawneado → el evento generado (`OnAudioSingleEnvelopeValue_Event_0`, firma `PlayingSoundWave, EnvelopeValue`) escribe **`VOEnvRaw`**.
- `StepVOReact(DT)` (en el Tick): `VOEnv` persigue a `VOEnvRaw` (interp 10) y `VOEnvRaw` decae solo a 0 (interp 2,5) para que al callar la voz la ameba se calme. Con `a = VOEnv × VOReact` empuja al MID del `Body`: **`WobbleAmount × (1 + 2a)`** (la deformación late con la voz) y **`Brightness × (1 + 0,8a)`** (el glow sube sin quemarse a blanco).
- 🎚 **Las perillas (cat. *I - Voz*, instance-editable):** `VOReact` (1,0) = **intensidad GLOBAL** (0 apaga todo; multiplica a las demás) · `VOReactWobble` (1,5) = cuánto late la **deformación** · `VOReactBright` (0,6) = cuánto sube el **brillo** · `VOReactSize` (0,08) = el **hinchado del cuerpo** (fracción del tamaño; 0,08 = hasta ~8% en los picos — sutil, respiración de un ser vivo). El pulso de tamaño escribe la escala del `Body` sólo cuando no está apareciendo/desapareciendo (los dueños de la escala no se pelean, § dos dueños).
- 🔴 Trampas pagadas: el evento del `Assign` escrito por DSL nace **SIN los parámetros** del delegate (el bind conecta pero el callback llega vacío) → el evento tipado lo genera **`create_node` del Assign**; y `SetEnvelopeFollowerAttackTime` es un **setter de propiedad**: el primer posicional es el VALOR y el target va segundo.

## 🕐 2026-08-21 — precisión del material y carga del VO
- `M_Alma`: **precisión Full + `Time.period = 300`** — mismo arreglo que `M_ProtoSoul` (ver `gotchas.md` §185): sin eso el material se degradaba a lo largo de la experiencia.
- 🔊 Los **32 VO pasaron a `loadingBehavior = ForceInline`**: con el valor heredado se cargaban del disco al reproducirse, y eso era el **corte que se sentía justo cuando Alma aparecía en cada sala** (el hitch no era la aparición, era el clip empezando). §186.
