...
✅ **PORTADO A LA GALERÍA el 2026-09-19** (`/Game/TestMeshes`, estación 13 `GAL_12`). Lo que se aprendió haciéndolo, que la ficha no decía:
  - **El "botón" del `Boot` es `BP_SaveMelody_SC`**, no un `BP_MenuButton`: `ButtonRef = GetActorOfClass(BP_SaveMelody_SC)`. Leer `Boot` ahorra la confusión.
  - **El CDO ya trae sembrado todo lo pesado**: los 20 `ModuleSounds`, `PadSound`, `NumSteps` 8 y los 4 tags. Una instancia nueva los hereda. Lo ÚNICO que hay que escribir a mano es **`StepIndex` 0..7 en los 8 slots** (`ZoneRadius` ya viene en 30, no en 0 como decía la ficha vieja).
  - **Sin `BP_Director_Story` no rompe**: `TellDirector` está envuelto en `IsValid`, simplemente no avisa el cierre.
  - **Sin panel de instrucciones tampoco rompe**: loguea *"el panel se fue sin confirmar - completo la intro"* y sigue derecho a las 20 esferas. Para una estación de galería sin panel, es justo lo que se quiere.
  - 🔴 **`WatchStage` cierra la mesa si `Phase` avanzó y el sensor NO está en modo 4.** Arrancar con `SeqIntro()` a secas NO alcanza: `ArmBeamNow` recién corre 2 s después (dentro de `SeqIntroGo`) y en ese hueco el vigilante cierra todo (`SEQ: la etapa cerro por fuera`). **El arranque tiene que hacer `SetStage(4)` + `MaybeInput()` ANTES de `SeqIntro()`, en el mismo frame.**
  - **`bAutoStart`** (nuevo en `0-Config`, **false** por defecto para no alterar la obra): con true, el secuenciador se arranca solo en el primer tick en que su estación se muestra — se apoya en que el director apaga el tick de las estaciones ocultas.
  - 🔴 **Sin panel, el paso 0 se dispara INVISIBLE.** `StartPad` sí pulsa el paso 0 en t=0 (no hay off-by-one en el contador), pero el `Marker` de cada slot **nace en escala 0** y crece con `RevealSpeed`. En la obra el panel mete segundos entre `ShowSlots` e `IntroDone`; sin panel toda la intro colapsa **en un frame** y el paso 0 pulsa con la mesa todavía invisible → el primero que se VE es el 1, y parece que el loop arranca en el segundo slot. Beltrán: *"me da la sensación de que el loop está iniciando en el slot_1... es un error que ya habíamos tenido en otras ocasiones"*.
    ✅ **`PadDelay`** (nuevo en `0-Config`, **0** por defecto = obra intacta): `IntroDone` arranca `StartPad` por timer en vez de en directo, así el pad y el paso 0 esperan a que la mesa esté visible. En la galería está en **0,9 s**. Como `StartPad` arranca el audio del pad **y** el reloj de pasos juntos, retrasarlo **no desalinea** nada.
    ⚠ **Trampa de la propia espera:** `TickSeq` acumula `StepTimer` en cuanto `Phase` llega a **2**, y `IntroDone` pone Phase 2 **antes** de la espera. Con `PadDelay` > `StepDur` el reloj alcanza a disparar **un paso de más antes de que arranque el pad** (se vio como "el segundo slot pulsa antes de que parta el primero"). ✅ `IntroDone` siembra `StepTimer = −PadDelay`: así llega a 0 justo cuando `StartPad` toma el control, y no dispara nada en el hueco. Funciona para cualquier `PadDelay`.
  - **Aceptación verificada en PIE**: `SEQ: boot slots=8` · `StepDur = 0.66666` · `SEQ: esferas=20` · `SEQ: pad ON, paso 0 alineado`.
...
🔴🔴 **EL UMBRAL ES QUIETUD + POSICIÓN, EN LAS DOS MECÁNICAS.** Esta ficha documentaba SOLO la caja de posición del pecho, y por esa omisión el manager extraído nació sin quietud: abría apenas la mano entraba en la caja, aunque estuvieras moviendo el brazo. Beltrán, 2026-09-19: *"Acuérdate que el umbral está definido por quietud más posición"*. **La quietud es la MISMA de respiración** (§4.7) y se porta tal cual: `StillLin` **14 cm/s** · `StillAng` **45 °/s** · `StillTau` **0,3** (EMA con ataque INSTANTÁNEO y caída suave: `max(v, ema)`), más los dos flags de tracking válido que devuelven `GetLinearVelocity`/`GetAngularVelocity`. ⚠ Eso obliga a que `HandR`/`HandL` sean **`MotionControllerComponent`** (`/Script/HeadMountedDisplay.MotionControllerComponent`), no `SceneComponent`: las velocidades salen del MotionController.
...
✅ **EXTRAÍDO el 2026-09-19 → [`BP_HeartManager_SC`](../.claude/skills/unreal-vr/blueprints/BP_HeartManager_SC.md)** (`Mechanics/Heart/`). Mismo patrón que el de respiración: cero pawn, cero director, cero input. Publica `bHeartZone`, `BeatCount`, `BeatEnv`, `HeartBPM` y **`OnHeartBeat`**. 🔴 Decisión clave (corregida en visor por Beltrán): **el umbral gatea TODO** — sin el mando en el pecho no hay latido, ni pulso, ni sonido, ni `OnHeartBeat`. Que el campo quede quieto es la obra, no un defecto. Primer consumidor: `BP_PulseField_SC`. 🔴 El dispatcher NO se puede llamar `OnBeatPulse`: colisiona con el de `BP_Sensor_Soul` y el DSL cablea el ajeno.
