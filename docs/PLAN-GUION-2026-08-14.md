...
- 🔜 **PRÓXIMO PASO — seguir el orden de obra: Acto 5, RECOGNIZING** (la subida por **10 pulsos** de igual extensión con curva trampolín, `SPulse` a ½ del BPM real por OSC, anillo Niagara por pulso desde el corazón, y fuera del umbral el avance sigue lento en vez de detenerse). Reemplaza `MaxBeatCount` y el descenso continuo actual de las columnas.
