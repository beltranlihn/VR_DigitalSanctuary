...
| 4 | **Attracting sin beam ni secuenciador**: aparecieron los objetos, pero **no apareció el beam** para interactuar y **no pulsaban los slots**. | 🟡 **Secuenciador ARREGLADO** (2026-08-14): `AttractRunBody` **nunca llamaba `StartExperience()`** del `BP_AttractDirector` — en el test aislado de Touch lo llamaba el `Finish()` del panel de instrucciones, que en la obra no corre. Cableado entre `EquipBeams` y `AttractIntro`, verificado por `get_node_infos` (el `read` lo etiqueta mal como `Class|BPStageDirector|StartExperience`). ⬜ **El beam sigue pendiente**: los 2 `BP_AimBeam` SÍ están en `L_Room_Attracting` y `EquipBeams` los equipa, así que lo más probable es que el problema sea el **visual** — hoy el beam se dibuja con `DrawDebugLine` (documentado como provisorio), que no es un visual válido para el visor. **Reusar `NS_MenuLaser` (XRFramework) o `BP_HandPointer`, que ya existen.** |
