...
| 🔨 **R5. Botón FINISH MELODY** | **CONSTRUIDO 2026-08-05.** `BP_SaveButton`: tag `Aimable`, mesh **con** colisión, **siempre presente** al centro de la fila de slots y 20 cm más abajo (`55, 0, 55`). 🔴 **Decisiones del usuario:** el botón **no aparece/desaparece** — está fijo y **crece** cuando los 5 slots se llenan; y se confirma con **hold de 3 s** del gatillo apuntándolo, no con un click. `RefreshAvailable` polea `Director.Slots` en el Tick (sin plumbing de notificación). Dispara el dispatcher `OnConfirmed`. El input lo alimenta `BP_AimBeam` por el `CastFailed` de `TryGrab`. Tracker: [`BP_SaveButton`](../../.claude/skills/unreal-vr/blueprints/BP_SaveButton.md). | **FALTA EL TEST:** con 4 slots está chico y el hold no hace nada. Al llenar el 5º **crece**. Lo apuntás + gatillo 3 s → crece progresivamente y confirma (log `TCH|FINISH MELODY confirmed`). Soltar antes o dejar de apuntarlo **cancela y vuelve a cero**. Sacás una burbuja → se vuelve a desactivar. |
