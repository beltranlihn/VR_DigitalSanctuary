...
| ✅ **R5. Botón FINISH MELODY** | **HECHO Y PROBADO EN VISOR 2026-08-05.** `BP_SaveButton`: tag `Aimable`, mesh **con** colisión, **siempre presente** al centro de la fila de slots y 20 cm más abajo (`55, 0, 55`). 🔴 **Decisiones del usuario:** el botón **no aparece/desaparece** — está fijo y **crece** cuando los 5 slots se llenan; y se confirma con **hold de 3 s** del gatillo apuntándolo, no con un click. `RefreshAvailable` polea `Director.Slots` en el Tick (sin plumbing de notificación). Dispara el dispatcher `OnConfirmed`. El input lo alimenta `BP_AimBeam` por el `CastFailed` de `TryGrab`. 🆕 **Al confirmar, cierra la mesa**: `ClearLooseBubbles` destruye toda burbuja con `bIsPlaced == false` y el botón se esconde (+ colisión y Tick apagados). Tracker: [`BP_SaveButton`](../../.claude/skills/unreal-vr/blueprints/BP_SaveButton.md). | ✅ **PROBADO:** con 4 slots está chico y el hold no hace nada. Al llenar el 5º crece; al apuntarlo crece un poco más. Gatillo sostenido 3 s → confirma. Soltar antes o dejar de apuntarlo cancela. Al confirmar desaparecen las esferas sueltas y el botón, y quedan las 5 de la mesa sonando. |
